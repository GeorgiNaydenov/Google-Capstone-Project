...
def poll_until_settled(
    api: TestClient, headers: dict[str, str], run_id: str, timeout: float = 2.0
) -> dict[str, object]:
...
def upload_asset(
    api: TestClient, headers: dict[str, str], patient_id: str | None = None
) -> str:
...
def test_health_demo_and_camel_case_read_contracts() -> None:
    ...
    dashboard = api.get(
        "/api/dashboard", headers=headers, params={"role": "clinician"}
    ).json()
    assert dashboard["metrics"]["patients"] > 0
    assert dashboard["patients"] and all(
        patient["id"] for patient in dashboard["patients"]
    )
    first = api.get("/api/patients", headers=headers).json()[0]
    matches = api.get(
        "/api/patients", headers=headers, params={"query": first["name"].split()[0]}
    ).json()
    ...
    assert (
        api.get(f"/api/sessions/{sessions[0]['id']}", headers=headers).json()[
            "occurredAt"
        ]
...
def test_extraction_is_review_gated_and_approval_persists() -> None:
    ...
    assert (
        api.get(f"/api/assets/{asset_id}", headers=headers).content
        == b"\x89PNG\r\n\x1a\nsynthetic"
    )
    baseline_persisted = len(
        api.get("/api/storage", headers=headers).json()["persistedExtractions"]
    )
    response = api.post(
        "/api/runs/extraction",
        headers=headers,
    ...
    assert {receipt["status"] for receipt in run["result"]["storageReceipts"]} == {
        "pending"
    }
    assert run["result"]["persisted"] is False
    assert (
        len(api.get("/api/storage", headers=headers).json()["persistedExtractions"])
        == baseline_persisted
    )
    ...
    approved = api.post(
        ...
        json={
            "decision": "approved",
            "fields": {"document_type": "Verified CT", "patient_match": patient_id},
        },
    ).json()
    assert approved["status"] == "completed"
    assert approved["result"]["persisted"] is True
    assert {receipt["status"] for receipt in approved["result"]["storageReceipts"]} == {
        "synced"
    }
    ...
    assert (
        mine["patientId"] == patient_id
        and mine["sessionId"] == approved["result"]["sessionId"]
    )
    ...
    sessions = api.get(
        "/api/sessions", headers=headers, params={"patient_id": patient_id}
    ).json()
    ...
    image_qa = api.post(
        ...
        json={
            ...
            "question": "Show approved imaging",
            "filters": {"source": "image", "dateRange": "30d"},
        },
    ).json()
    ...
    assert (
        api.get(
            image_qa["evidence"][0]["sourceUrl"].replace("approval", "expired")
        ).status_code
        == 403
    )
...
def test_rejection_never_persists_extraction() -> None:
    ...
    rejected = api.post(
        f"/api/runs/{run['id']}/review", headers=headers, json={"decision": "rejected"}
    ).json()
    assert rejected["result"]["persisted"] is False
    assert {receipt["status"] for receipt in rejected["result"]["storageReceipts"]} == {
        "pending"
    }
    assert (
        len(api.get("/api/storage", headers=headers).json()["persistedExtractions"])
...
def test_qa_database_admin_and_configuration_contracts() -> None:
    ...
    assert api.get(f"/api/runs/{qa_run['id']}", headers=clinical_headers).json()[
        "result"
    ]["answer"]
    clinician_preview = api.post(
        "/api/runs/database/preview",
        headers=clinical_headers,
        json={"question": "Count risk groups"},
    )
    assert clinician_preview.status_code == 201
    assert (
        api.post(
            f"/api/runs/database/{clinician_preview.json()['id']}/execute",
            headers=clinical_headers,
        ).status_code
        == 200
    )
    ...
    executed = api.post(
        f"/api/runs/database/{preview['id']}/execute", headers=admin_headers
    ).json()
    assert (
        executed["status"] == "completed"
        and executed["result"]["chart"]["type"] == "bar"
    )
    ...
    assert (
        saved["version"] == config["version"] + 1
        and saved["autoApprovalThreshold"] == 94
    )
    users = api.get("/api/users", headers=admin_headers).json()
    assert any("Admin" in user["roles"] for user in users) and all(
        user["scope"] for user in users
    )
    metrics = api.get(
        "/api/dashboard", headers=admin_headers, params={"role": "admin"}
    ).json()["metrics"]
    assert {
        "activeUsers",
        "agentRuns",
...
def test_run_history_and_knowledge_base_listing_restore_conversations() -> None:
    ...
    qa = api.post(
        ...
        json={
            "patientId": patient_id,
            "question": "Trend since last session?",
            "filters": {},
        },
    )
    assert qa.status_code == 201
    preview = api.post(
        "/api/runs/database/preview",
        headers=headers,
        json={"question": "Count patients by risk level"},
    )
    assert preview.status_code == 201
    assert (
        api.post(
            f"/api/runs/database/{preview.json()['id']}/execute", headers=headers
        ).status_code
        == 200
    )

    all_runs = api.get("/api/runs", headers=headers).json()
    assert qa.json()["id"] in {run["id"] for run in all_runs}
    assert preview.json()["id"] in {run["id"] for run in all_runs}
    assert {"qa", "database"} <= {run["workflow"] for run in all_runs}
    qa_runs = api.get(
        "/api/runs",
        headers=headers,
        params={"workflow": "qa", "patient_id": patient_id},
    ).json()
    restored = next(run for run in qa_runs if run["id"] == qa.json()["id"])
    assert restored["result"]["question"] == "Trend since last session?"
    assert (
        api.get(
            "/api/runs",
            headers=headers,
            params={"workflow": "qa", "patient_id": "PT-0000"},
        ).json()
        == []
    )
    other_session = api.get("/api/runs", headers=clinician("history-other")).json()
    assert qa.json()["id"] not in {run["id"] for run in other_session}
    assert any(run.get("seededConversation") for run in other_session)
    ...
    listed = api.get(
        "/api/knowledge-base/assets", headers=headers, params={"patient_id": patient_id}
    ).json()
...
def test_agent_catalog_and_notifications_are_actionable() -> None:
    ...
    assert {pipeline["id"] for pipeline in catalog["pipelines"]} == {
        "extraction",
        "qa",
        "database",
    }
    ...
    updated = api.post(
        f"/api/notifications/{notices[0]['id']}/read", headers=request_headers
    )
    assert updated.status_code == 200 and updated.json()["read"] is True
    assert (
        api.get("/api/notifications", headers=request_headers).json()[0]["read"] is True
    )
...
def test_demo_session_state_isolation_and_reset() -> None:
    ...
    assert (
        len(api.get("/api/storage", headers=clinician("one")).json()["assets"])
        == baseline + 1
    )
    assert (
        len(api.get("/api/storage", headers=clinician("two")).json()["assets"])
        == baseline
    )
...
def test_orchestration_classifies_context_and_audits_plan() -> None:
    ...
    extraction = api.post(
        ...
        headers=headers,
        json={
            "query": "Extract findings from this uploaded CT scan",
            "patientId": roster[0]["id"],
        },
    )
    ...
    qa = api.post(
        ...
        headers=headers,
        json={
            "query": "What changed in recent evidence?",
            "patientId": roster[1]["id"],
        },
    ).json()
    ...
    assert (
        api.post(
            "/api/orchestrate",
            headers=headers,
            json={"query": "Summarize recent evidence"},
        ).status_code
        == 422
    )
    assert (
    ...
    database = api.post(
        "/api/orchestrate",
        headers=headers,
        json={"query": "Count population risk cohorts"},
    ).json()
...
def test_upload_limit_and_repository_eviction() -> None:
    ...
    api = client()
    patient_id = api.get("/api/patients", headers=clinician("upload-limit")).json()[0][
        "id"
    ]
    oversized = api.post(
        "/api/assets",
        headers=clinician("upload-limit"),
        data={"patient_id": patient_id},
...
def test_tenant_header_selects_distinct_demo_datasets() -> None:
    ...
    assert (
        research_patients[0]["condition"] != northstar_patients[0]["condition"]
        or research_patients[0]["id"] != northstar_patients[0]["id"]
    )
    ...
    assert (
        api.post("/api/notifications/NTF-001/read", headers=research).status_code == 200
    )
    assert (
        sum(
            not n["read"]
            for n in api.get("/api/notifications", headers=research).json()
        )
        == 2
    )
    assert (
        sum(
            not n["read"]
            for n in api.get("/api/notifications", headers=northstar).json()
        )
        == 3
    )
...
def test_legacy_tenant_values_map_to_research_clinic() -> None:
    ...
    expected = api.get(
        "/api/patients",
        headers={**clinician("legacy-reference"), "X-Tenant": "research-clinic"},
    ).json()
...
def test_permissions_matrix_default_and_admin_edit() -> None:
    ...
    saved = api.put(
        "/api/permissions", headers=admin(), json={"matrix": edited["matrix"]}
    ).json()
...
def test_report_schedules_round_trip_and_audit() -> None:
    ...
    saved = api.put(
        "/api/report-schedules/cohort-risk",
        headers=clinician(),
    ...
    rows = api.get("/api/report-schedules", headers=clinician()).json()
    assert (
        next(row for row in rows if row["id"] == "cohort-risk")["frequency"] == "weekly"
    )
...
def test_summary_counts_reflect_repository_state() -> None:
    ...
    baseline_runs = summary["runs"]
    patient_id = api.get("/api/patients", headers=headers).json()[0]["id"]
    asset_id = upload_asset(api, headers, patient_id)
    run = api.post(
        "/api/runs/extraction",
        headers=headers,
        json={"patientId": patient_id, "uploadIds": [asset_id]},
    ).json()
    assert run["status"] == "review"
    refreshed = api.get("/api/summary", headers=headers).json()
    assert refreshed["runs"] == baseline_runs + 1
...
def test_patient_evidence_endpoint_returns_sources() -> None:
    ...
    rows = next(
        (
            ...
            for items in (
                api.get(
                    f"/api/patients/{patient['id']}/evidence", headers=headers
                ).json()
                for patient in roster[:100]
            )
            if items
        ),
        None,
...
def test_storage_records_derive_from_upload_and_approval() -> None:
    ...
    before = {
        record["id"]
        for record in api.get("/api/storage", headers=headers).json()["records"]
    }
    asset_id = upload_asset(api, headers, patient_id)
    after_upload = api.get("/api/storage", headers=headers).json()["records"]
    assert any(
        record["destination"] == "Object storage"
        and record["source"] == "scan.png"
        and record["id"] not in before
        for record in after_upload
    )
    run = api.post(
        "/api/runs/extraction",
        headers=headers,
    ...
    api.post(
        f"/api/runs/{run['id']}/review",
        headers=headers,
        json={"decision": "approved", "fields": {"document_type": "CT"}},
    )
    records = api.get("/api/storage", headers=headers).json()["records"]
    extraction_records = [
        record for record in records if record["source"] == f"Extraction {run['id']}"
    ]
    assert {record["destination"] for record in extraction_records} == {
        "JSON document store",
        "Relational database",
        "Vector search index",
    }
    assert all(
        record["status"] == "synced" and record["patientId"] == patient_id
...
def test_audit_event_detail_links_seeded_run() -> None:
    ...
    assert (
        detail["run"]["id"] == "RUN-SEED-DB" and detail["run"]["status"] == "completed"
    )
    assert (
        api.get("/api/audit/AUD-UNKNOWN", headers=admin("audit-detail")).status_code
        == 404
    )
    linked = api.get("/api/runs/RUN-SEED-DB", headers=clinician("audit-detail")).json()
    assert linked["workflow"] == "database" and linked["steps"]
    # Bootstrap provenance stays hidden; curated demo conversations remain
    # visible so a fresh browser can resume prior patient Q&A.
    conversations = api.get("/api/runs", headers=clinician("audit-detail")).json()
    assert conversations and all(run.get("seededConversation") for run in conversations)
    assert "RUN-SEED-DB" not in {run["id"] for run in conversations}


@requires_generated_dataset
def test_demo_qa_returns_renderable_image_and_restorable_answer_package() -> None:
    """Patient Q&A keeps the multimodal evidence in both answer and history."""

    api = client()
    headers = clinician("multimodal-history")
    seeded = api.get("/api/runs", headers=headers, params={"workflow": "qa"}).json()
    prior = next(run for run in seeded if run.get("result", {}).get("imageEvidence"))
    patient_id = prior["result"]["patientId"]
    response = api.post(
        "/api/runs/qa",
        headers=headers,
        json={
            "patientId": patient_id,
            "question": "What changed and what should I review?",
            "filters": {},
        },
    )
    assert response.status_code == 201
    run = response.json()
    image = run["result"]["imageEvidence"]
    assert image["kind"] == "image" and image["sourceUrl"].startswith("/api/assets/")
    assert any(row["type"] == "image" for row in run["result"]["summaryRows"])
    assert run["result"]["answerSections"]["recommendedAction"]
    restored = api.get(
        "/api/runs",
        headers=headers,
        params={"workflow": "qa", "patient_id": patient_id},
...
@requires_generated_dataset
def test_extraction_packet_keeps_selected_patient_and_full_packet_lineage() -> None:
    """Guided extraction exposes every packet record without changing review scope."""

    api = client()
    headers = clinician("packet-lineage")
    sources = api.get("/api/extraction/sources", headers=headers).json()
    source = next(item for item in sources if item["patientsInFile"] >= 5)
    response = api.post(
        "/api/runs/extraction",
        headers=headers,
        json={"patientId": source["patientId"], "assetId": source["assetId"]},
    )
    assert response.status_code == 201
    extracted = response.json()["result"]["extractedContent"]
    assert extracted["selectedPatientId"] == source["patientId"]
    assert len(extracted["packetRecords"]) == source["patientsInFile"]
    assert {record["patientId"] for record in extracted["packetRecords"]} == set(
        source["batchPatientIds"]
    )
    assert response.json()["result"]["fields"]["patient_match"] == source["patientId"]


@requires_generated_dataset
def test_population_polypharmacy_returns_prevalence_chart_and_clinical_copy() -> None:
    """Polypharmacy response carries rows, denominator, prevalence, and chart axes."""

    api = client()
    headers = clinician("population-prevalence")
    preview = api.post(
        "/api/runs/database/preview",
        headers=headers,
        json={"question": "Which patients are on 8 or more active medications?"},
    )
    assert preview.status_code == 201
    assert "prevalence_percent" in preview.json()["result"]["sql"]
    executed = api.post(
        f"/api/runs/database/{preview.json()['id']}/execute", headers=headers
    )
    assert executed.status_code == 200
    result = executed.json()["result"]
    assert result["rows"] and result["population"]["matchingPatients"] >= len(
        result["rows"]
    )
    assert (
        result["population"]["totalPatients"]
        >= result["population"]["matchingPatients"]
    )
    assert result["chart"]["y"] == "active_medications"
    assert "%" in result["clinicalSummary"] and result["recommendedAction"]
...
def test_demo_storage_serves_sample_records_with_patient_context() -> None:
    ...
    sample = [
        ...
        if str(record["id"]).startswith("SEED-RCP-")
    ]
    assert sample and all(
        record["patientId"] and record["sessionId"] for record in sample
    )
    seeded = [
        entry
        for entry in storage["persistedExtractions"]
        if str(entry["runId"]).startswith("RUN-SEED-")
    ]
    assert seeded and all(
        entry["jsonReceipt"] and entry["patientId"] and entry["confidence"] is not None
        for entry in seeded
    )
...
def test_database_examples_are_grounded_and_answerable() -> None:
    ...
    preview = api.post(
        "/api/runs/database/preview", headers=headers, json={"question": questions[0]}
    )
    assert preview.status_code == 201
    executed = api.post(
        f"/api/runs/database/{preview.json()['id']}/execute", headers=headers
