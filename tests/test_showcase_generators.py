...
def test_extraction_generator_creates_uploadable_pdf_packets(tmp_path: Path) -> None:
    ...
    assert {row["pipeline"] for row in contract["agentMonitoringSeed"]} == {"extraction"}
...
def test_multimodal_generator_creates_pdf_and_knowledge_base_corpus(tmp_path: Path) -> None:
    ...
    assert {row["pipeline"] for row in manifest["frontend_contract"]["agentMonitoringSeed"]} == {"qa"}
...
def test_database_generator_creates_four_year_sqlite_showcase(tmp_path: Path) -> None:
    ...
    assert manifest["frontend_contract"]["storageSeed"]["relationalRows"] == manifest["total_rows"]
    assert manifest["frontend_contract"]["dashboardSeed"]["patients"] == 5
    assert manifest["frontend_contract"]["dashboardSeed"]["databaseRows"] == manifest["total_rows"]
    assert manifest["frontend_contract"]["dashboardSeed"]["completeness"] > 0
    assert {row["pipeline"] for row in manifest["frontend_contract"]["agentMonitoringSeed"]} == {"database"}
    ...
    assert any(trace["type"] == "heatmap" and trace.get("showscale") for trace in charted)
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        schema_tables = {item[0] for item in conn.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()}
    assert row[0] == manifest["row_counts"]["sessions"]
    assert row[1] <= "2022-07-10"
    assert row[2] <= "2026-07-05"
    assert {"patient_conditions", "medications", "vital_signs", "care_gaps", "social_determinants", "insurance_policies", "appointments", "immunizations", "family_history"} <= schema_tables
...
def test_generated_showcase_dataset_drives_demo_dashboard_stats(tmp_path: Path) -> None:
    ...
    from scripts.generate_multimodal_patient_showcase import generate as generate_multimodal
    from clinical_app.repository import DemoRepository, load_showcase_dataset

    base = tmp_path / "showcase"
    generate_database(base / "database" / "clinical_showcase.db", base / "database", patient_count=6, seed=21, replace=True, years=1, anchor_date=date(2026, 7, 5))
    generate_extraction(base / "extraction", count=3, seed=22, frontend_public=None)
    generate_multimodal(base / "multimodal", bundle_count=1, days=7, comparators=3, seed=23, pdfs_per_bundle=2, kb_docs_per_bundle=5)
    ...
    assert dataset.dashboard_seed["storedAssets"] == dataset.storage_seed["cloudObjects"]
    assert dataset.storage_seed["relationalRows"] > dataset.dashboard_seed["patients"]
    assert {"database", "extraction", "qa"} <= {row["pipeline"] for row in dataset.monitor_baseline}
    repo = DemoRepository(dataset)
    extraction_uploads = [item for item in repo.uploads.values() if item.get("sourceUse") == "extraction"]
    assert extraction_uploads and {item["contentType"] for item in extraction_uploads} == {"application/pdf"}
    ...
    assert all(repo.runs[run_id]["seeded"] and repo.runs[run_id]["status"] == "completed" for run_id in ("RUN-SEED-DB", "RUN-SEED-EXT", "RUN-SEED-QA"))
