...
def test_v2_mcp_tools_are_retired() -> None:
    """Public deployment must not expose callable MCP services."""
    api = client()
    headers = clinician()

    res = api.get("/api/v2/mcp/tools", headers=headers)
    assert res.status_code == 410
    assert "disabled" in res.json()["detail"].casefold()

    execution = api.post(
        "/api/v2/mcp/execute",
        headers=headers,
        json={"toolName": "get_patient_status", "arguments": {}},
    )
    assert execution.status_code == 410


def test_v2_a2a_card_is_retired() -> None:
    """Public deployment must not advertise an ADK agent to other callers."""
    api = client()
    headers = clinician()
...
def test_retired_live_tenant_falls_back_to_deterministic_demo() -> None:
    """Forged retired live headers must never select a paid agent runtime."""
    api = client()
    headers = {**clinician("retired-live"), "X-Tenant": "capstone"}
