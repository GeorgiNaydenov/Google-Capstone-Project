...
First, it is a Google ADK multi-agent system. The backend contains 22 LLM agents: one root orchestrator plus 21 specialists. Three `SequentialAgent` containers organize the workflows, and extraction contains a nested `LoopAgent`. The image extraction pipeline has seven specialist LLM agents for quality assessment, OCR processing, vision analysis, clinical structuring, critic/refiner validation, and review-request preparation. The patient Q&A pipeline has eight specialists for validation, context assembly, evidence retrieval, image evidence, citation building, answer synthesis, audit, and response assembly. The database intelligence pipeline has six specialists for schema discovery, natural-language-to-SQL generation, SQL validation, approval preparation, execution, and insight/chart generation. The root agent routes intent across these workflows and exposes a coherent clinical interaction surface.
...
Fourth, the project is deployable. The repository includes a production Dockerfile, a Cloud Build pipeline for Cloud Run, canonical `/health` and `/ready` endpoints, and a Vertex AI Agent Engine configuration. The public Cloud Run build serves the React frontend and FastAPI backend as a single-origin deterministic product service with paid ADK/Gemini execution disabled. A self-controlled deployment can enable live ADK/Gemini mode with explicit credentials and runtime configuration. The deployment notes also document the persistence boundary: until state is moved to external managed storage, Cloud Run should run with one instance and durable real-tenant data should be externalized.
...
There are two execution paths. The public Cloud Run build is deterministic and requires no API key, so judges can run the product workflows without depending on paid external services. In a self-controlled deployment, live mode switches the Capstone tenant to the real Google ADK and Gemini execution path when credentials are configured. The live tenant starts empty and fills only with what is actually uploaded, approved, and run. That separation is deliberate: the public build shows the product story reliably, while the repository preserves a real live-agent boundary for configured deployments.

The project is also explicit about what is real and what is emulated. Real components include the ADK agent graph, MCP server, Pydantic tool contracts, SQLite persistence, document parsing, human-in-the-loop approval gates, tool-call traces, audit logs, security callbacks, model-tier registry, Cloud Run containerization, `/health`, `/ready`, and live-mode bridging to Gemini. Local demo equivalents stand in for some cloud services such as object storage, vector search, and Firestore-like state. Those parts are labelled as local demo implementations rather than being oversold as managed production infrastructure.
...
The deployment path is intentionally honest rather than magical. The Cloud Run container builds the frontend, installs the Python agent/product runtime, and serves everything from FastAPI. `/health` is the liveness probe, while `/ready` checks real dependencies such as database access, upload writability, frontend assets, and agent/MCP imports. The deployment documentation calls out the current persistence boundary: Cloud Run file systems are ephemeral, so real tenant data should use `CLINICAL_DATA_DIR` on persistent storage if it must survive revisions. That caveat is visible because production-readiness includes knowing what is not production-grade yet.
