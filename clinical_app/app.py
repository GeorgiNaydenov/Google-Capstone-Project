...
from clinical_app.agent_runtime import (
    database_execution_tools,
    database_preview_tools,
    extraction_review_tools,
    extraction_tools,
    qa_tools,
)
...
from clinical_app.repository import (
    DemoRepository,
    LiveRepository,
    RepositoryRegistry,
    now,
...
def _rank_patient_evidence(
    repo: DemoRepository | LiveRepository,
    patient_id: str,
    question: str,
    source_filter: list[str],
    date_range: str,
) -> list[dict[str, Any]]:
    """Rank patient evidence by relevance, recency, and multimodal utility."""

    candidates = [
        item
        for item in repo.evidence.get(patient_id, [])
        if not source_filter or item.get("source_type") in source_filter
    ]
    if date_range in {"30d", "1y"}:
        cutoff = datetime.now(UTC).date() - timedelta(
            days=30 if date_range == "30d" else 365
        )
        candidates = [
            item
            for item in candidates
            if item.get("date")
            and datetime.fromisoformat(str(item["date"])).date() >= cutoff
        ]
    ...
    def score(item: dict[str, Any]) -> tuple[float, str]:
        text_terms = set(re.findall(r"[a-z0-9]+", str(item.get("text", "")).casefold()))
        overlap = len(question_terms & text_terms)
        source_type = item.get("source_type")
        visual_boost = (
            5.0
            if source_type == "image" and wants_visual
            else 0.75
            if source_type == "image"
            else 0.0
        )
        return overlap + visual_boost, str(item.get("date", ""))

    ranked = sorted(candidates, key=score, reverse=True)
    selected = ranked[:7]
    if not source_filter or "image" in source_filter:
        image = next(
            (item for item in ranked if item.get("source_type") == "image"), None
        )
        if image and image not in selected:
            selected = [*selected[:6], image]
    return selected
...
def _evidence_views(
    ...
    """Map stored evidence into reopenable frontend citations."""
    ...
    return [
        {
            "id": item["source_id"],
            "label": f"{item['source_type'].title()} evidence - {item['date']}",
            "kind": kind_map.get(item["source_type"], item["source_type"]),
            "excerpt": item.get("text", ""),
            **(
                {
                    "sourceUrl": f"/api/assets/{item['asset_id']}?session={repo.session_id}"
                }
...
def _clinical_qa_package(
    patient: dict[str, Any],
    question: str,
    evidence_views: list[dict[str, Any]],
) -> dict[str, Any]:
    """Build a concise clinician-facing answer package from grounded evidence."""

    text_sources = [item for item in evidence_views if item.get("kind") != "image"]
    image_source = next(
        (
            item
            for item in evidence_views
            if item.get("kind") == "image" and item.get("sourceUrl")
        ),
        None,
    )
    lead = str((text_sources[0] if text_sources else {}).get("excerpt", "")).strip()
    lead_sentence = next(
        (part.strip() for part in re.split(r"(?<=[.!?])\s+", lead) if part.strip()), ""
    )
    diagnosis = str(patient.get("primary_diagnosis") or "the documented condition")
    risk = str(patient.get("risk_level") or "not classified").replace("_", " ")
    if lead_sentence:
        direct_answer = (
            f"For {patient['name']}, the record supports {diagnosis} with current risk status {risk}. "
            f"Most relevant recent evidence: {lead_sentence}"
        )
    else:
        direct_answer = f"No authorized evidence matched the selected filters for {patient['name']}."
    if image_source:
        direct_answer += " The attached patient-specific visual trend should be interpreted alongside the cited authored record."
    recommended_action = (
        "Review the cited source and reconcile any material change with the current medication list and care plan before acting."
        if evidence_views
        else "Broaden the evidence filters or index the missing source before making a clinical inference."
    )
    summary_rows = [
        {
            "id": f"summary-{index}",
            ...
            "file": item["label"],
            "type": item["kind"],
            "finding": str(item.get("excerpt", ""))[:240],
        }
        for index, item in enumerate(evidence_views, 1)
    ]
    return {
        "answer": direct_answer,
        "imageEvidence": image_source,
        "summaryRows": summary_rows,
        "answerSections": {
            "keyEvidence": lead_sentence or "No matching evidence was available.",
            "recommendedAction": recommended_action,
            "limitations": "This answer summarizes authorized evidence and does not replace clinician review or source verification.",
        },
        "question": question,
    }


def _medication_prevalence_sql(question: str, sql: str) -> str:
    """Add an explicit cohort denominator to the polypharmacy demo query."""

    normalized = question.casefold()
    if "8 or more" not in normalized or "medication" not in normalized:
        return sql
    return """WITH eligible AS (
    SELECT p.patient_id, p.name, COUNT(*) AS active_medications, p.age, p.risk_level
    FROM medications m
    ...
    SELECT COUNT(*) AS total_patients FROM patients_core
)
SELECT e.*, COUNT(*) OVER () AS matching_patients, c.total_patients,
       ROUND(100.0 * COUNT(*) OVER () / c.total_patients, 2) AS prevalence_percent
FROM eligible e CROSS JOIN cohort c
ORDER BY e.active_medications DESC
LIMIT 15"""


def _population_result_package(
    question: str, rows: list[dict[str, Any]], columns: list[str]
) -> dict[str, Any]:
    """Derive chart axes, prevalence, and clinical interpretation from SQL rows."""

    first = rows[0] if rows else {}
    numeric_columns = [
        column
        for column in columns
        if isinstance(first.get(column), (int, float))
        and column not in {"matching_patients", "total_patients"}
    ]
    label_columns = [column for column in columns if column not in numeric_columns]
    x_axis = (
        label_columns[0] if label_columns else columns[0] if columns else "category"
    )
    y_axis = (
        numeric_columns[0]
        if numeric_columns
    ...
    if rows and all(
        key in first
        for key in ("matching_patients", "total_patients", "prevalence_percent")
    ):
        population = {
            "matchingPatients": int(first["matching_patients"]),
            "totalPatients": int(first["total_patients"]),
            "prevalencePercent": float(first["prevalence_percent"]),
            "denominator": "all authorized patients",
        }
    if population:
        summary = (
            f"{population['matchingPatients']} of {population['totalPatients']} authorized patients "
            f"({population['prevalencePercent']:.2f}%) meet the requested criterion. "
            "Use this as a medication-reconciliation worklist, not an automatic treatment recommendation."
        )
    elif rows:
        summary = f"The governed query returned {len(rows)} result rows for: {question}"
    else:
        summary = "The governed query completed, but no patients matched the requested criterion."
    return {
        "chart": {"type": "bar", "x": x_axis, "y": y_axis, "title": question}
        if rows
        else None,
        "population": population,
        "clinicalSummary": summary,
        "recommendedAction": "Review the highest-burden patients first and verify medication status in the source record.",
        "limitations": "Prevalence uses the authorized tenant cohort as denominator and reflects current synthetic demo data.",
    }


def patient_view(
    item: dict[str, Any], data_sources: int | None = None
) -> dict[str, Any]:
    ...
    return {
        "id": item["patient_id"],
        "name": item["name"],
        "mrn": f"MRN-{item['patient_id'][3:]}",
        "age": item.get("age"),
        "sex": item.get("sex"),
        "condition": item.get("primary_diagnosis"),
        ...
        else 3 + int(item.get("open_tasks", 0) > 0),
...
def tenant_patient_view(repo: Any, item: dict[str, Any]) -> dict[str, Any]:
    ...
    return patient_view(
        item, data_sources=len(repo.evidence.get(item["patient_id"], []))
    )
...
def audit_view(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "actor": item["actor"],
        "entity": entity,
        "result": details.get("result", "recorded"),
    }
...
def create_app() -> FastAPI:
    ...
    api = FastAPI(
        title="Clinical AI Command Center API",
        version="0.3.0",
        docs_url=None,
        redoc_url=None,
    ...
    COMMON_RESPONSES = {
        400: {
            "model": ErrorResponse,
            "description": "Bad Request: Parameter verification or query syntax error",
        },
        401: {
            "model": ErrorResponse,
            "description": "Unauthorized: Session key invalid or authorization headers missing",
        },
        403: {
            "model": ErrorResponse,
            "description": "Forbidden: User role does not possess permissions to write/read the target resource",
        },
        404: {
            "model": ErrorResponse,
            "description": "Not Found: The target patient, session, database run, or image asset does not exist",
        },
        422: {
            "model": ErrorResponse,
            "description": "Unprocessable Entity: Parameter validation constraints failed",
        },
        500: {
            "model": ErrorResponse,
            "description": "Internal Server Error: Persisted SQLite storage connection error or agent execution crash",
        },
    }
    ...
    def effective_mode(tenant: TenantConfig) -> str:
        ...
        Public product deployments are permanently deterministic. Tenant
        resolution only returns demo tenants, and this second guard makes the
        paid-service shutdown explicit at the API boundary.
        """
    ...
    async def context(
        ...
        if getattr(repo, "db_path", None) is not None:
            ...
            with database.tenant_storage(
                repo.db_path, getattr(repo, "uploads_root", None)
            ):
    ...
    Context = Annotated[
        tuple[DemoRepository | LiveRepository, Role, str, TenantConfig],
        Depends(context),
    ]
    ...
    def guard_model_request(repo: LiveRepository, text: str) -> str:
        ...
        if active_model_runs[key] >= config["max_concurrent_model_runs"]:
            ...
            raise HTTPException(
                429, "Too many active model runs; wait for a run to finish"
            )
    ...
    def patient_or_404(
        repo: DemoRepository | LiveRepository, patient_id: str
    ) -> dict[str, Any]:
    ...
    def upload_patient_or_register(
        repo: DemoRepository | LiveRepository, patient_id: str
    ) -> dict[str, Any]:
    ...
    def run_or_404(
        repo: DemoRepository | LiveRepository, run_id: str
    ) -> dict[str, Any]:
    ...
    def author_steps_to_rows(
        run_id: str, author_steps: list[dict[str, Any]], running_last: bool = False
    ) -> list[dict[str, Any]]:
        ...
        steps = [
            {
                "id": f"{run_id}-S{i}",
                "name": str(step.get("author", "root_agent")).replace("_", " ").title(),
                "status": "running" if running_last and i == len(rows) else "completed",
                "detail": "ADK event completed",
                "timestamp": now(),
            }
    ...
    def attach_live_metadata(
        result: dict[str, Any], live_result: dict[str, Any]
    ) -> None:
    ...
    def last_tool_output(
        live_result: dict[str, Any], tool_name: str
    ) -> dict[str, Any] | None:
        ...
        for call in reversed(live_result.get("toolCalls", [])):
            if (
                ...
                and call["output"].get("status") == "success"
            ):
    ...
    @v1_router.get(
        ...
        tags=["Health"],
        responses={
            200: {"description": "Report liveness for V1 API"},
            **COMMON_RESPONSES,
        },
    )
    ...
    def ready() -> dict[str, Any]:
        ...
        degraded = [
            c["name"]
            for c in components
            if c["status"] != "operational" and c["name"] in blocking
        ]
        payload = {
            "status": "not-ready" if degraded else "ready",
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "Expose the real ADK pipeline catalog and tenant execution mode"
            },
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        tags=["Notifications"],
        responses={
            200: {"description": "List of unresolved notifications from agents"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        tags=["Health"],
        responses={
            200: {"description": "Measured component health for the caller's tenant"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        "/agents/monitoring",
        response_model=list[AgentMonitorRow],
        tags=["Agents"],
        responses={
            200: {"description": "Per-agent runtime statistics"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        tags=["Admin"],
        responses={200: {"description": "Role-permission matrix"}, **COMMON_RESPONSES},
    )
    ...
    @v1_router.put(
        ...
        tags=["Admin"],
        responses={
            200: {"description": "Persisted the edited permission matrix"},
            **COMMON_RESPONSES,
        },
    )
    def save_permissions(
        body: PermissionsUpdateRequest, ctx: Context
    ) -> dict[str, Any]:
    ...
    @v1_router.get(
        "/database/schema",
        response_model=list[SchemaTable],
        tags=["Database"],
        responses={
            200: {
                "description": "Clinical database tables parsed from the governed DDL"
            },
            **COMMON_RESPONSES,
        },
    )
        ...
        for table, ddl in SCHEMA_DDL.items():
            ...
            for line in ddl.splitlines()[1:]:
                ...
                if match and match.group(1).upper() not in {
                    ...
                    columns.append(
                        {"name": match.group(1), "type": match.group(2).upper()}
                    )
    ...
    @v1_router.get(
        ...
        tags=["Dashboard"],
        responses={
            200: {"description": "Live workspace counts for navigation badges"},
            **COMMON_RESPONSES,
        },
    )
    def summary(ctx: Context) -> dict[str, Any]:
        ...
        return {
            "queueCount": sum(
                item.get("ai_review_status") == "needs_review"
                for item in repo.patients.values()
            ),
            "inboxCount": sum(
                item["workflow"] == "extraction" and item["status"] == "review"
                for item in repo.runs.values()
            ),
            "unreadNotifications": sum(
                not item.get("read") for item in repo.build_notifications()
            ),
    ...
    @v1_router.get(
        "/patients/{patient_id}/evidence",
        response_model=list[EvidenceItemResponse],
        tags=["Patients"],
        responses={
            200: {"description": "Evidence sources for one patient"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.post(
        ...
        tags=["Notifications"],
        responses={
            200: {"description": "Marked one notification as read"},
            **COMMON_RESPONSES,
        },
    )
    def read_notification(notification_id: str, ctx: Context) -> dict[str, Any]:
        ...
        notification = next(
            (item for item in ctx[0].notifications if item["id"] == notification_id),
            None,
        )
        if not notification:
    ...
    @v1_router.post(
        ...
        tags=["Demo"],
        responses={
            201: {"description": "Created an isolated deterministic demo session"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.post(
        ...
        tags=["Demo"],
        responses={
            204: {"description": "Reset the caller's deterministic demo session"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "Retrieved global metrics and recent activity dashboard details"
            },
            **COMMON_RESPONSES,
        },
    )
    def dashboard(ctx: Context, role: Role | None = None) -> dict[str, Any]:
        ...
        completeness = round(
            sum(float(p["completeness"] or 0) for p in patients)
            / max(1, len(patients))
            * 100
        )
        pending = sum(p["aiStatus"] == "needs_review" for p in patients)
        persisted = sum(
            item["workflow"] == "extraction" and item["result"].get("persisted")
            for item in repo.runs.values()
        )
        extraction_count = sum(
            item["workflow"] == "extraction" for item in repo.runs.values()
        )
        sync_rate = (
            round(persisted / extraction_count * 100) if extraction_count else 100
        )
        ...
        pending_actions = (
            pending
            + sum(item["status"] == "review" for item in repo.runs.values())
            + sum(not item.get("read") for item in repo.build_notifications())
        )
        if getattr(repo, "dashboard_seed", None) is not None:
            ...
            metrics: dict[str, int | str] = {
                ...
                "highRisk": ds.get(
                    "highRiskEstimate", sum(p["risk"] == "high" for p in patients)
                ),
                "pendingReview": ds.get("pendingReviewEstimate", pending),
                "pendingVerifications": ds.get(
                    "pendingVerifications", ds.get("pendingReviewEstimate", pending)
                ),
                "imageExtractionsToday": ds.get(
                    "imageExtractionsToday", ds.get("sourceImages", len(sessions))
                ),
                "openAiAlerts": ds.get(
                    "openAiAlerts",
                    len([item for item in repo.notifications if not item["read"]]) + 2,
                ),
                ...
                "pendingActions": ds.get("pendingReviewEstimate", pending)
                + ds.get("failedExtractions", failed)
                + len(repo.runs)
                + len([item for item in repo.notifications if not item["read"]]),
            ...
            metrics = {
                ...
                "highRisk": sum(p["risk"] == "high" for p in patients),
                "pendingReview": pending,
                "pendingVerifications": pending,
                "imageExtractionsToday": len(sessions),
                "openAiAlerts": len(
                    [item for item in repo.notifications if not item["read"]]
                )
                + alert_base,
                "agentRuns24h": run_base + len(repo.runs),
                "syncRate": sync_rate,
            ...
            if active_role == "admin":
                metrics.update(
                    {
                        "activeUsers": len(users),
                        "agentRuns": len(repo.runs),
                        "reviewSla": "4m",
                        "auditEvents": len(repo.audit),
                        "storedAssets": len(repo.uploads),
                    }
                )
        return {
            "metrics": metrics,
            "patients": patients[:8],
            "sessions": sessions[:8],
            "activity": [audit_view(item) for item in reversed(repo.audit[-8:])],
        }
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "List of matched patient profiles according to criteria"
            },
            **COMMON_RESPONSES,
        },
    )
    def patients(
        ctx: Context, query: str = "", q: str = "", risk: str | None = None
    ) -> list[dict[str, Any]]:
        ...
        if needle:
            rows = [
                ...
                if needle
                in " ".join(
                    (p["patient_id"], p["name"], p["primary_diagnosis"])
                ).casefold()
            ]
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "Retrieved detailed patient profile and current risk metadata by ID"
            },
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        tags=["Sessions"],
        responses={
            200: {"description": "List of recent clinical extraction sessions"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "Retrieved detailed session status and confidence scores"
            },
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.post(
        ...
        tags=["Assets"],
        responses={
            201: {"description": "Asset uploaded and recorded successfully"},
            **COMMON_RESPONSES,
        },
    )
    async def create_asset(
        ctx: Context, file: UploadFile = File(...), patient_id: str = Form("")
    ) -> dict[str, Any]:
        ...
        if not repo.is_demo:
            ...
            if (
                not patient_id
                and not detected_id
                and str(extracted.get("type")) == "image"
            ):
                ...
                try:
                    ...
                    detected_id = detect_patient_id(
                        str(pre_extraction.get("text") or "")
                    )
                except Exception:
                    pre_extraction = None
            if not patient_id:
                if not detected_id:
                    ...
                    raise HTTPException(
                        422,
                        "patientId is required: none was provided and none could be detected in the document",
                    )
            ...
            try:
                res = process_document(
                    dest_path, patient_id, pre_extraction=pre_extraction
                )
                if "error" not in res:
                    extracted["textPreview"] = res.get(
                        "text_preview", extracted.get("textPreview", "")
                    )
                    extracted["pageCount"] = res.get(
                        "page_count", extracted.get("pageCount", 1)
                    )
            except Exception:
                preview_text = (
                    extracted.get("textPreview") or f"Document text fallback for {fn}"
                )
                ...
                chunk_dicts = [
                    {"index": c["index"], "text": c["text"], "page": 1} for c in chunks
                ]
    ...
    @v1_router.post(
        ...
        responses={
            201: {
                "description": "Uploaded and indexed a patient-scoped knowledge-base document"
            },
            **COMMON_RESPONSES,
        },
    )
    async def create_knowledge_base_asset(
        ...
        repo.evidence.setdefault(patient_id, []).insert(
            0,
            {
                "source_id": asset_id,
                "source_type": "document",
                "date": now()[:10],
                "text": f"{fn}: {preview_text}",
                "asset_id": asset_id,
                "filename": fn,
            },
        )
        repo.log(
            "knowledge_base_asset_uploaded",
            user,
            role,
            patient_id=patient_id,
            asset_id=asset_id,
    ...
    @v1_router.post(
        ...
        responses={
            201: {
                "description": "Ingested and went through the ETL of a PDF, image, or database intelligence cohort"
            },
            **COMMON_RESPONSES,
        },
    )
    async def import_data(
        ...
        if repo.is_demo:
            raise HTTPException(
                400, "Import is only supported in live (real tenant) mode."
            )
        ...
        if import_type == "document":
            ...
            repo.log(
                "asset_imported", user, role, patient_id=patient_id, asset_id=asset_id
            )
            return {
                "status": "success",
            ...
            try:
                ...
                repo.log(
                    "database_intelligence_imported",
                    user,
                    role,
                    details=json.dumps(row_counts),
                )
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "List indexed knowledge-base documents, optionally scoped to one patient"
            },
            **COMMON_RESPONSES,
        },
    )
    def knowledge_base_assets(
        ctx: Context, patient_id: str | None = None
    ) -> list[dict[str, Any]]:
        ...
        for item in repo.uploads.values():
            ...
            servable = item["assetId"] in repo.asset_contents or bool(
                item.get("filePath")
            )
            view["previewUrl"] = (
                f"/api/assets/{item['assetId']}?session={repo.session_id}"
                if servable
                else ""
            )
            rows.append(view)
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "List generated extraction packet sources for demo picking"
            },
            **COMMON_RESPONSES,
        },
    )
    def extraction_sources(ctx: Context) -> list[dict[str, Any]]:
        ...
        for item in repo.uploads.values():
            ...
            if item.get("previewPath"):
                demo_platform = item.get("demoPlatform") or (
                    "demo2" if tenant.id == "northstar" else "primary"
                )
            ...
            rows.append(
                {
                    ...
                    "label": f"{patient.get('name', item.get('patientId', 'Patient'))} ({item.get('patientId', '')}) - {item.get('packetId', item['assetId'])}",
                    ...
                    "reviewRequired": any(
                        field.get("needs_review")
                        for field in item.get("extracted", {}).get("fields", [])
                    ),
                }
            )
        return rows
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "Serve uploaded evidence bytes for authorized preview"
            },
            **COMMON_RESPONSES,
        },
    )
    def asset(asset_id: str, ctx: Context, session: str | None = None) -> Response:
        ...
        if asset_id in repo.asset_contents:
            ...
            return StreamingResponse(
                iter([repo.asset_contents[asset_id]]),
    ...
    @v1_router.post(
        ...
        responses={
            201: {
                "description": "Began deterministic extraction pipeline on target report assets"
            },
            **COMMON_RESPONSES,
        },
    )
    async def extraction(body: RunRequest, ctx: Context) -> dict[str, Any]:
        ...
        if not asset_ids or any(
            asset_id not in repo.uploads
            or repo.uploads[asset_id].get("patientId") != patient_id
        ...
        def asset_kind(asset_meta: dict[str, Any]) -> str:
            return (
                "document"
                if asset_meta.get("contentType") == "application/pdf"
                else "image"
            )
        ...
        if not repo.is_demo:
            ...
            file_mime = repo.uploads[asset_ids[0]].get(
            ...
            item = {
                ...
                "steps": [
                    {
                        ...
                        "name": "Agent Pipeline",
                        "status": "running",
                        "detail": "Routing to the clinical evidence extraction pipeline",
                        "timestamp": now(),
                    }
                ],
                ...
                "result": {
                    ...
                    "storageReceipts": [],
                    "persisted": False,
                    "extractedContent": extracted_meta,
                },
                "review": None,
            }
            repo.record_run(item)
            ...
            async def run_in_background() -> None:
                try:
                    live_result = await execute_live(
                        ...
                        on_step=lambda steps: item.update(
                            steps=author_steps_to_rows(run_id, steps, running_last=True)
                        ),
                    )
                    fields = live_result.get("fields", {})
                    if not fields:
                        final_response = str(
                            live_result.get("finalResponse", "")
                        ).strip()
                            ...
                            item["steps"].append(
                                {
                                    ...
                                    "status": "error",
                                    "detail": "Extraction produced no structured fields or narrative response",
                                    "timestamp": now(),
                                }
                            )
                            return
                        fields = {
                            "document_type": "Clinical evidence",
                            "patient_match": patient_id,
                            "finding": final_response,
                        }
                    item["steps"] = live_steps(
                        run_id, live_result, "Clinical Review Gate", "review"
                    )
                    item["status"] = "review"
                    ...
                    item["result"]["storageReceipts"] = [
                        {"target": t, "status": "pending"}
                        for t in ("json", "relational", "vector")
                    ]
        ...
        extracted_fields = (
            extracted_meta.get("fields", [])
            if isinstance(extracted_meta.get("fields"), list)
            else []
        )
        fields = {
            str(field.get("field_name") or field.get("label") or f"field_{index}"): (
                f"{field.get('value')} {field.get('unit')}".strip()
                if field.get("unit")
        ...
        if not fields:
            fields = {
                "document_type": extracted_meta.get(
                    "documentType", "Clinical evidence"
                ),
                "patient_match": patient_id,
                "finding": str(
                    extracted_meta.get("textPreview")
                    or "Evidence ready for clinician verification"
                ),
            }
        confidence = (
            round(
                sum(float(field.get("confidence", 0.88)) for field in extracted_fields)
        ...
        steps = [
            {
                "id": f"{run_id}-S{i}",
                "name": name,
                "status": "completed" if i < 6 else "review",
                "detail": "Specialist stage completed"
                if i < 6
                else "Awaiting clinician decision",
                "timestamp": now(),
            }
        ...
        item = {
            ...
            "result": {
                ...
                "storageReceipts": [
                    ...
                    for target in ("json", "relational", "vector")
                ],
                "persisted": False,
                "extractedContent": extracted_meta,
            },
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "Retrieved status, confidence scores, and current stage details of an active pipeline run"
            },
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "Retrieved list of completed run step execution events"
            },
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        tags=["Runs"],
        responses={
            200: {"description": "Server-Sent Events (SSE) stream of run progress"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.post(
        ...
        responses={
            200: {
                "description": "Recorded review decision to approve or reject structured extraction data"
            },
            **COMMON_RESPONSES,
        },
    )
    def review(run_id: str, body: ReviewRequest, ctx: Context) -> dict[str, Any]:
        ...
        if body.field_updates:
            item["result"]["fields"] = body.field_updates.get(
                "fields", body.field_updates
            )
        ...
        review_traces = extraction_review_tools(
            pid,
            run_id,
            body.decision,
            user,
        ...
        if approved:
            if repo.is_demo:
                ...
                trace_status = {
                    trace["tool"]: trace.get("status") for trace in review_traces
                }
                receipt_tools = {
                    "json": "store_to_gcs",
                ...
                item["result"]["storageReceipts"] = [
                    {
                        "target": target,
                        "status": "synced"
                        if trace_status.get(tool) == "success"
                        else "failed",
                        "receiptId": repo.identifier("RCP"),
                    }
                    for target, tool in receipt_tools.items()
                ]
            item["result"]["persisted"] = all(
                receipt["status"] == "synced"
                for receipt in item["result"]["storageReceipts"]
            )
            ...
            repo.sessions[session_id] = {
                ...
                "date": occurred_at,
                "uploaded_image_count": len(item["evidence"]),
            ...
            evidence_rows.append(
                {
                    ...
                    "date": occurred_at,
                    "text": json.dumps(item["result"]["fields"], sort_keys=True),
                }
            )
            for source in item["evidence"]:
                evidence_rows.append(
                    {
                        "source_id": source["id"],
                        "source_type": source.get("kind", "image"),
                        "date": occurred_at,
                        "text": f"Clinician-approved source: {source['label']}",
                        "asset_id": source["id"],
                    }
                )
        ...
        item["steps"].append(
            {
                ...
                "status": "completed",
                "detail": "Approved and persisted"
                if approved
                else "Rejected; no extracted result persisted",
                "timestamp": now(),
            }
        )
        repo.log(
            f"extraction_{body.decision}",
            user,
            role,
            patient_id=item["result"]["patientId"],
            run_id=run_id,
            result="persisted" if approved else "not_persisted",
        )
        repo.record_run(item)
        return item
    ...
    @v1_router.get(
        ...
        tags=["Runs"],
        responses={
            200: {"description": "List extraction runs awaiting clinician review"},
            **COMMON_RESPONSES,
        },
    )
    def reviews(ctx: Context) -> list[dict[str, Any]]:
        ...
        return [
            item
            for item in ctx[0].runs.values()
            if item["workflow"] == "extraction" and item["status"] == "review"
        ]
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "List this session's agent runs for conversation-history restore"
            },
            **COMMON_RESPONSES,
        },
    )
    def list_runs(
        ctx: Context, workflow: str | None = None, patient_id: str | None = None
    ) -> list[dict[str, Any]]:
        ...
        rows = [
            item
            for item in repo.runs.values()
            if not item.get("seeded")
            and (not workflow or item.get("workflow") == workflow)
        ]
        if patient_id:
            rows = [
                item
                for item in rows
                if item.get("result", {}).get("patientId") == patient_id
            ]
        for item in rows:
            for source in item.get("evidence", []):
                if source.get("id") in repo.uploads:
                    source["sourceUrl"] = (
                        f"/api/assets/{source['id']}?session={repo.session_id}"
                    )
            image_evidence = item.get("result", {}).get("imageEvidence")
            if (
                isinstance(image_evidence, dict)
                and image_evidence.get("id") in repo.uploads
            ):
                image_evidence["sourceUrl"] = (
                    f"/api/assets/{image_evidence['id']}?session={repo.session_id}"
                )
        return rows
    ...
    @v1_router.post(
        ...
        responses={
            201: {
                "description": "Classify user query and structure orchestrator execution route"
            },
            **COMMON_RESPONSES,
        },
    )
    def orchestrate(body: OrchestrateRequest, ctx: Context) -> dict[str, Any]:
        ...
        if workflow in {"extraction", "qa"}:
            if not body.patient_id:
                raise HTTPException(
                    422, "patientId is required for patient-specific workflows"
                )
            patient_or_404(repo, body.patient_id)
        plan = {
            "intent": intent,
    ...
    @v1_router.post(
        ...
        responses={
            201: {
                "description": "Began patient-grounded QA retrieval pipeline execution"
            },
            **COMMON_RESPONSES,
        },
    )
    ...
    async def question(body: QuestionRequest, ctx: Context) -> dict[str, Any]:
        ...
        repo, role, user, _tenant = ctx
        model_slot = (
            guard_model_request(repo, body.question) if not repo.is_demo else ""
        )
        if repo.is_demo:
        ...
        run_id = repo.identifier("RUN")
        audit = repo.log(
            "question_answered", user, role, patient_id=body.patient_id, run_id=run_id
        )

        if not repo.is_demo:
            source_types_str = (
                ",".join(body.source_types) if body.source_types else "all"
            )
            source_filter = [
                {
                    "pdf": "document",
                ...
                for value in body.source_types
            ]
            date_range = body.filters.get("dateRange", "all")
            local_evidence = _rank_patient_evidence(
                repo, body.patient_id, body.question, source_filter, date_range
            )
            evidence_views = _evidence_views(repo, local_evidence)
            answer_package = _clinical_qa_package(
                repo.patients.get(
                    body.patient_id, {"name": f"Patient {body.patient_id}"}
                ),
                body.question,
                evidence_views,
            )
                ...
                "steps": [
                    {
                        ...
                        "name": "Agent Pipeline",
                        "status": "running",
                        "detail": "Routing to the patient Q&A pipeline",
                        "timestamp": now(),
                    }
                ],
            ...
            async def run_in_background() -> None:
                try:
                    ...
                    item["result"]["answer"] = state_outputs.get(
                        "qa_answer"
                    ) or live_result.get("finalResponse", "No answer generated")
                    attach_live_metadata(item["result"], live_result)
                    ...
                    citations = (
                        last_tool_output(live_result, "build_citations") or {}
                    ).get("citations", [])
                    if citations:
        ...
        source_filter = [
            {"pdf": "document", "json": "document", "knowledge_base": "document"}.get(
                item, item
            )
            for item in source_filter
        ]
        date_range = body.filters.get("dateRange", "all")
        evidence = _rank_patient_evidence(
            repo, body.patient_id, body.question, source_filter, date_range
        )
        evidence_views = _evidence_views(repo, evidence)
        answer_package = _clinical_qa_package(patient, body.question, evidence_views)
        ...
        item = {
            ...
            "status": "completed",
            "agentName": "patient_qa_pipeline",
            "confidence": 0.88 if evidence else 0.0,
            "createdAt": now(),
            "auditId": audit["audit_id"],
            "traceId": f"TRACE-{run_id}",
            "steps": [
                {
                    ...
                    "name": name,
                    "status": "completed",
                    "detail": f"Retrieved {len(evidence_views)} sources"
                    if name == "Retrieval Agent"
                    else "Stage completed",
                    "timestamp": now(),
                }
                for i, name in enumerate(qa_step_names, 1)
            ],
            "evidence": evidence_views,
            "result": {
                **answer_package,
    ...
    @v1_router.post(
        "/runs/database/preview",
        status_code=201,
        tags=["Database"],
        responses={
            201: {
                "description": "Began database cohort preview pipeline to classify intent and compile SQL query"
            },
            **COMMON_RESPONSES,
        },
    )
    async def database_preview(body: DatabaseRequest, ctx: Context) -> dict[str, Any]:
        ...
        if not repo.is_demo:
            item = {
                ...
                "workflow": "database",
                "status": "running",
                "agentName": "db_intelligence_pipeline",
                "createdAt": now(),
                "auditId": audit["audit_id"],
                "traceId": f"TRACE-{run_id}",
                "steps": [
                    {
                        ...
                        "name": "Agent Pipeline",
                        "status": "running",
                        "detail": "Routing to the population insights pipeline",
                        "timestamp": now(),
                    }
                ],
                "evidence": [],
                        ...
                        item["steps"].append(
                            {
                                ...
                                "name": "SQL Generation",
                                "status": "error",
                                "detail": "Agent did not return SQL for this question",
                                "timestamp": now(),
                            }
                        )
                        return
                    item["steps"] = live_steps(
                        run_id, live_result, "SQL Approval Gate", "review"
                    )
                    item["status"] = "review"
                    item["result"]["sql"] = sql
                    ...
                    item["steps"] = [
                        {
                            ...
                            "name": "Agent Pipeline",
                            "status": "error",
                            "detail": f"SQL generation failed: {type(exc).__name__}: {exc}",
                            "timestamp": now(),
                        }
                    ]
                finally:
        ...
        if query_cards:
            ...
            for card in query_cards:
                card_words = {
                    word.strip("?.!,").casefold()
                    for word in str(card.get("question", "")).split()
                    if len(word.strip("?.!,")) > 3
                }
                scored.append((len(submitted_words & card_words), card))
            selected_card = max(scored, key=lambda item: item[0])[1]
        sql = str(
            (selected_card or {}).get("sql")
            or "SELECT risk_level, COUNT(*) AS patient_count FROM patients_core GROUP BY risk_level ORDER BY patient_count DESC"
        )
        sql = _medication_prevalence_sql(body.question, sql)
        ...
        item = {
            ...
            "steps": [
                {
                    "id": f"{run_id}-S1",
                    "name": "Schema Understanding Agent",
                    "status": "completed",
                    "timestamp": now(),
                },
                {
                ...
                {
                    "id": f"{run_id}-S4",
                    "name": "SQL Approval Gate",
                    "status": "review",
                    "detail": "Awaiting explicit execution approval",
                    "timestamp": now(),
                },
            ],
            "evidence": [],
            "result": {
                "question": body.question,
                "matchedQuestion": (selected_card or {}).get("question"),
                "sql": sql,
                "safe": bool(verdict["safe"]),
                "readOnly": True,
                "tables": verdict.get("tables_referenced", []),
                "toolCalls": tool_calls,
            },
        }
        repo.record_run(item)
    ...
    @v1_router.post(
        "/runs/database/{run_id}/execute",
        tags=["Database"],
        responses={
            200: {
                "description": "Executed approved SQL query and saved result table and charts"
            },
            **COMMON_RESPONSES,
        },
    )
    async def database_execute(run_id: str, ctx: Context) -> dict[str, Any]:
        ...
        if not verdict["safe"]:
            item["steps"].append(
                {
                    "id": f"{run_id}-S{len(item['steps']) + 1}",
                    "name": "Query execution",
                    "status": "error",
                    "detail": f"SQL rejected: {verdict['reason']}",
                    "timestamp": now(),
                }
            )
            ...
            item["steps"].append(
                {
                    ...
                    "name": "Query execution",
                    "status": "review",
                    "detail": f"Execution failed: {res['error']}",
                    "timestamp": now(),
                }
            )
        ...
        item["result"].update(
            {
                "columns": columns,
                "rows": rows,
                **_population_result_package(
                    item["result"].get("question", ""), rows, columns
                ),
            }
        )
        item["result"]["toolCalls"].extend(
            database_execution_tools(
                item["result"].get("question", ""), sql, rows, user
            )
        )
        item["steps"].append(
            {
    ...
    @v1_router.get(
        ...
        tags=["Database"],
        responses={
            200: {"description": "Retrieved database cohort run history"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "Example population questions grounded in the tenant's actual database contents"
            },
            **COMMON_RESPONSES,
        },
    )
    def database_examples(ctx: Context) -> list[str]:
        ...
        if repo.is_demo:
            ...
            questions = [
                str(card.get("question")) for card in cards if card.get("question")
            ]
        ...
        condition = probe(
            "SELECT condition_name, COUNT(DISTINCT patient_id) AS n FROM patient_conditions GROUP BY condition_name ORDER BY n DESC LIMIT 1"
        )
        if condition and condition[0].get("condition_name"):
            examples.append(
                f"Which patients have {str(condition[0]['condition_name']).rstrip('.').lower()}, and how old are they?"
            )
        critical = probe(
            "SELECT COUNT(*) AS n FROM lab_results WHERE flag LIKE 'critical%'"
        )
        ...
        panel = probe(
            "SELECT test_name, COUNT(*) AS n FROM lab_results GROUP BY test_name ORDER BY n DESC LIMIT 1"
        )
        if panel and panel[0].get("test_name"):
            examples.append(
                f"Show the most recent {panel[0]['test_name']} result date for each patient"
            )
        noshow = probe(
        ...
        if vitals and int(vitals[0].get("n") or 0):
            examples.append(
                "What is the average systolic blood pressure by risk level?"
            )
        return examples[:7] or fallback

    @v1_router.get(
        "/database/queries/{run_id}/csv",
        tags=["Database"],
        responses={
            200: {"description": "Retrieved query results exported as a CSV stream"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        tags=["Storage"],
        responses={
            200: {"description": "Retrieved storage system metrics and totals"},
            **COMMON_RESPONSES,
        },
    )
    def storage(ctx: Context) -> dict[str, Any]:
        ...
        persisted = [
            persisted_view(item)
            for item in repo.runs.values()
            if item["workflow"] == "extraction" and item["result"].get("persisted")
        ]
        destinations = {
        ...
        for item in repo.runs.values():
            ...
            for receipt in item["result"].get("storageReceipts", []):
                records.append(
                    {
                        "id": receipt.get("receiptId")
                        or f"{item['id']}-{receipt['target']}",
                        "source": f"Extraction {item['id']}",
                        "destination": destinations.get(
                            receipt["target"], receipt["target"]
                        ),
                        ...
                        else "Persistence tool reported failure",
                    }
                )
        if getattr(repo, "storage_seed", None) is not None:
            ...
            sample_sessions = sorted(
                repo.sessions.values(),
                key=lambda item: str(item.get("date", "")),
                reverse=True,
            )[:10]
            receipt_targets = (
                ("JSON", "json", "JSON document store", "Structuring agents"),
                ("SQL", "relational", "Relational database", "Database generator"),
                ("VECTOR", "vector", "Vector search index", "Embedding agents"),
            )
                        ...
                        "receipts": [
                            {
                                "target": target,
                                "status": "synced",
                                "receiptId": f"SEED-RCP-{tag}-{index:03d}",
                            }
                            for tag, target, _destination, _owner in receipt_targets
                        ],
                        "fields": {
                            str(field.get("field_name", f"field_{position}")): str(
                                field.get("value", "")
                            )
                            for position, field in enumerate(
                                session.get("extracted_fields", []), 1
                            )
                        },
                    }
                )
            seeded_records = [
                (
                    "SEED-CLOUD",
                    f"Provisioned: {ss.get('cloudObjects', 0)} generated evidence files",
                    "Object storage",
                    "Showcase data generator",
                ),
                (
                    "SEED-AUDIT",
                    f"Provisioned: {ss.get('auditEvents', 0)} generated audit events",
                    "Audit log",
                    "Audit service",
                ),
            ]
        ...
        return {
            ...
            "persistedExtractions": persisted,
            "records": records,
            "assetCount": len(repo.uploads),
            "persistedCount": len(persisted),
            "cloudCount": len(repo.uploads),
            "jsonCount": len(persisted),
            "sqlCount": len(persisted),
            "vectorCount": len(persisted),
            "auditCount": len(repo.audit),
        }
    ...
    @v1_router.get(
        ...
        tags=["Admin"],
        responses={
            200: {"description": "Retrieved corporate user directory"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "Retrieved current agent validation thresholds and concurrent limits"
            },
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.put(
        ...
        responses={
            200: {
                "description": "Updated and persisted new agent thresholds configuration"
            },
            **COMMON_RESPONSES,
        },
    )
    def save_agent_config(config: dict[str, Any], ctx: Context) -> dict[str, Any]:
        ...
        if unknown:
            raise HTTPException(
                422, f"Unknown configuration keys: {', '.join(sorted(unknown))}"
            )
        repo.agent_config.update(
            {key: value for key, value in config.items() if key != "version"}
        )
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "List every report with its recurring generation schedule"
            },
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.put(
        ...
        tags=["Reports"],
        responses={
            200: {"description": "Updated the report's recurring generation frequency"},
            **COMMON_RESPONSES,
        },
    )
    def save_report_schedule(
        report_id: str, body: ReportScheduleUpdateRequest, ctx: Context
    ) -> dict[str, Any]:
        ...
        item["updatedAt"] = now()
        item["nextRun"] = (
            (datetime.now(UTC) + timedelta(days=interval_days)).strftime("%Y-%m-%d")
            if interval_days
            else None
        )
        repo.log(
    ...
    @v1_router.get(
        ...
        tags=["Storage"],
        responses={
            200: {"description": "Serve patient structured visual image binary data"},
            **COMMON_RESPONSES,
        },
    )
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "Immutable compliance activity and security log history"
            },
            **COMMON_RESPONSES,
        },
    )
    def audit(ctx: Context, patient_id: str | None = None) -> list[dict[str, Any]]:
        ...
        rows = [
            ...
            if not patient_id or item["details"].get("patient_id") == patient_id
        ]
    ...
    @v1_router.get(
        ...
        responses={
            200: {
                "description": "Retrieved one audit event with its linked run and patient context"
            },
            **COMMON_RESPONSES,
        },
    )
    def audit_event(audit_id: str, ctx: Context) -> dict[str, Any]:
        ...
        repo = ctx[0]
        item = next(
            (entry for entry in repo.audit if entry["audit_id"] == audit_id), None
        )
        if item is None:
        ...
        view["run"] = (
            {
                ...
                "steps": [
                    ...
                    for step in run_item.get("steps", [])
                ],
            }
            if run_item
            else None
        ...
        view["patient"] = (
            {"id": patient.get("patient_id", ""), "name": patient.get("name", "")}
            if patient
            else None
        )
        return view
    ...
    @v2_router.get(
        ...
        responses={
            200: {
                "description": "Retrieve health diagnostic metrics including database connection check"
            },
            **COMMON_RESPONSES,
        },
    )
    ...
    @v2_router.get(
        ...
        tags=["MCP V2"],
        responses={
            200: {"description": "Expose all database tools available over MCP"},
            **COMMON_RESPONSES,
        },
    )
    async def list_mcp_tools() -> dict[str, Any]:
        """Reject MCP discovery: public deployment has no callable MCP service."""
        raise HTTPException(410, "MCP service is disabled in this public deployment.")
    ...
    @v2_router.post(
        ...
        tags=["MCP V2"],
        responses={
            200: {"description": "Gated dynamic execution of an MCP tool"},
            **COMMON_RESPONSES,
        },
    )
    async def execute_mcp_tool(body: McpExecuteRequest, ctx: Context) -> dict[str, Any]:
        """Reject MCP execution before any tool or agent module is imported."""
        raise HTTPException(410, "MCP execution is disabled in this public deployment.")

    @v2_router.get(
        "/a2a/card",
        response_model=A2aCardResponse,
        tags=["A2A V2"],
        responses={
            200: {
                "description": "Expose the Agent Card metadata for Agent-to-Agent discovery"
            },
            **COMMON_RESPONSES,
        },
    )
    ...
    @v2_router.get(
        ...
        responses={
            200: {
                "description": "Expose a list of categories and files for both the Obsidian wiki and Karpathy LLM Wiki"
            },
            **COMMON_RESPONSES,
        },
    )
    def docs_list() -> dict[str, Any]:
            ...
            for root, dirs, files in os.walk(base_dir):
                ...
                for file in files:
                    ...
                    try:
                        ...
                        pass
                    files_list.append(
                        {"path": rel_path.replace("\\", "/"), "title": title}
                    )
            return files_list
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to list documentation files: {e}"
            )
    ...
    @v2_router.get(
        ...
        responses={
            200: {
                "description": "Retrieve content of an Obsidian or Karpathy wiki file"
            },
            **COMMON_RESPONSES,
        },
    )
    ...
    if dist.is_dir():
        ...
        def spa(path: str) -> FileResponse:
            ...
            if path in ("api", "docs", "redoc", "openapi.json") or path.startswith(
                ("api/", "docs/", "redoc/")
            ):
