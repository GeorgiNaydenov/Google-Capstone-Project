...
def extraction_tools(patient_id: str, asset_id: str, run_id: str) -> list[dict[str, Any]]:
    ...
    review_flag = flag_for_review("finding", "Evidence ready for clinician verification", 0.87)
    ...
    return [_trace("extract_clinical_text", ocr), _trace("flag_for_review", review_flag), _trace("log_audit_event", audit)]
...
def qa_tools(patient_id: str, question: str, source_types: list[str], date_range: str) -> list[dict[str, Any]]:
    ...
    evidence_items = notes.get("data", {}).get("results", []) + vectors.get("data", {}).get("results", [])
    ...
    audit = log_audit_event(
        ...
        "question_answered",
        patient_id,
        json.dumps({"source_types": source_filter, "source_count": len(evidence_items)}),
    )
    return [
...
def database_preview_tools(question: str, sql: str) -> list[dict[str, Any]]:
    ...
    generated = generate_sql(question, schema.get("data", {}).get("schema_ddl", "patients"))
    safety = validate_sql_safety(sql)
    audit = log_audit_event(
        "sql_validator_agent",
        "database_preview_generated",
        details=json.dumps({"question": question, "safe": safety.get("data", {}).get("safe", False)}),
    )
    return [_trace("get_database_schema", schema), _trace("generate_sql", generated), _trace("validate_sql_safety", safety), _trace("log_audit_event", audit)]


def database_execution_tools(question: str, sql: str, rows: list[dict[str, Any]], approver: str) -> list[dict[str, Any]]:
    ...
    chart = generate_chart_spec(json.dumps({"columns": list(rows[0]) if rows else [], "rows": rows}), question)
    audit = log_audit_event(
        "insight_chart_agent",
        "database_query_executed",
        details=json.dumps({"approver": approver, "row_count": len(rows)}),
    )
    return [_trace("approve_sql_preview", approval), _trace("execute_approved_clinical_query", execution), _trace("generate_chart_spec", chart), _trace("log_audit_event", audit)]
