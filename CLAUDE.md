...
Shared durable harness context: @.claude/memory/project.md
...
| `.claude/skills/drawio-skill/SKILL.md` | Wrapper for the vendored upstream drawio-skill package |

### Agents

| Agent | Ownership |
|---|---|
| `.claude/agents/harness-governor.md` | Hooks, sync, harness contracts, and state lifecycle |
| `.claude/agents/memory-state-reviewer.md` | ADK state, memory governance, compaction, and A2A boundaries |
| `.claude/agents/security-reviewer.md` | Read-only security boundary and data-leak review |
| `.claude/agents/verification-runner.md` | Deterministic checks, pytest, lint, and eval gate reporting |
...
| `.claude/commands/harness-status.md` | Shows current lifecycle state and recent handoffs |
| `.claude/commands/handoff.md` | Writes a sanitized machine-readable agent handoff |
...
| `.claude/references/state-contract.md` | Ephemeral state schema and lifecycle transitions |
...
| `.claude/memory/README.md` | Memory location and ownership guide |
| `.claude/memory/project.md` | Shared durable harness facts loaded at session boundaries |
| `.claude/memory/handoff-protocol.md` | Cross-agent return contract |
| `.claude/agent-memory/*/MEMORY.md` | Project-scoped learned memory for named subagents |
| `.claude/state/` | Gitignored ephemeral lifecycle state and handoff queue |
