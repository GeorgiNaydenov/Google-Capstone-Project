...
INJECTION_PATTERNS: list[tuple[str, str]] = [
    ...
    (
        r"(reveal|show|display|output|print)\s+(me\s+)?(your\s+)?(system\s+)?prompt",
        "prompt_extraction",
    ),
    (
    ...
    (
        r"(enter|switch\s+to|enable)\s+(developer|debug|admin|root|god)\s+mode",
        "privilege_escalation",
    ),
    (
        r"bypass\s+(your\s+)?(safety|security|filter|restriction|guardrail)",
        "bypass_attempt",
    ),
    ...
    (
        r"(do\s+not|don't)\s+(follow|obey|listen\s+to)\s+(your\s+)?(rules|instructions)",
        "bypass_attempt",
    ),
    # Clinical / HIPAA bypass attempts
    (
        r"(ignore|disregard|bypass|disable)\s+(hipaa|privacy|audit|compliance|phi)\s+(requirement|restriction|check|protocol|rule)",
        "hipaa_bypass",
    ),
    (
        r"(list|show|dump|export)\s+(all\s+)?patient\s+(names|records|data|identifiers)",
        "phi_extraction",
    ),
    (
        r"(disable|turn\s+off|skip)\s+(pii|phi|redact|audit)\s+(filter|detection|logging|trail)",
        "safety_disable",
    ),
]

_COMPILED_PATTERNS = [
    (re.compile(p, re.IGNORECASE), label) for p, label in INJECTION_PATTERNS
]
...
OUT_OF_SCOPE_PATTERNS: list[tuple[str, str]] = [
    (
        r"(?<!\w)(?:python|javascript|typescript|java|rust|golang|c\+\+|c#)(?!\w)",
        "software_programming",
    ),
    (
        r"\b(?:write|debug|compile|refactor)\b.{0,40}\b(?:code|program|script|function|class)\b",
        "software_programming",
    ),
...
PHI_PATTERNS: list[tuple[str, str, re.Pattern]] = [
    ...
    ("icd10_code", "ICD-10 diagnosis code", re.compile(r"\b[A-Z]\d{2}\.\d{1,4}\b")),
    (
        "npi_number",
        "provider NPI number",
...
SECRET_PATTERNS: list[tuple[str, re.Pattern]] = [
    ...
    (
        "generic_secret",
        re.compile(
            r"(?:secret|token|credential)\s*[:=]\s*['\"]?[\w\-]{16,}['\"]?",
            re.IGNORECASE,
        ),
    ),
]
...
def redact_detected_secrets(text: str) -> str:
    """Replace every secret pattern recognized by ``scan_for_secrets``.

    Unlike ``config.redact_secrets``, which targets key/value log fields, this
    function also removes standalone provider keys and private-key headers.
    Memory and delegation boundaries use it after detection so recognized
    values cannot remain in persisted nested state.
    """
    result = text
