...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate second-platform multimodal Q&A demo bundles.")
    ...
    manifest = generate(args.output, args.bundle_count, args.days, args.comparators, args.seed, args.pdfs_per_bundle, args.kb_docs_per_bundle, args.template, "demo2", args.patient_prefix)
    print(json.dumps({"output": str(args.output), "bundle_count": manifest["bundle_count"], "pdf_count": manifest["pdf_count"], "knowledge_base_document_count": manifest["knowledge_base_document_count"], "demo_platform": "demo2"}, indent=2))
