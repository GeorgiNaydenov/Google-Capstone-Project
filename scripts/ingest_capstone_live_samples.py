...
def _severity_to_risk_level(severity: str | None) -> str:
    ...
    return {"Severe": "high", "Moderate": "needs_review", "Mild": "stable"}.get(severity or "", "needs_review")
...
def _normalize_flag(abnormal_flag: str | None) -> str:
    ...
    """
    return {"H": "high", "HH": "critical_high", "L": "low", "LL": "critical_low"}.get(abnormal_flag or "", "normal")
...
def ingest(source_sqlite: Path, db_path: Path, uploads_dir: Path) -> dict[str, int]:
    ...
    counts = {
        "providers": 0, "patients_core": 0, "patient_conditions": 0,
        "medications": 0, "lab_results": 0, "lab_panels": 0,
        "vital_signs": 0, "procedures": 0, "clinical_notes": 0,
    }
    ...
    with capstone_db.tenant_storage(db_path, uploads_dir):
        ...
        with capstone_db.get_connection() as conn:
            for p in providers:
                conn.execute(
                    ...
                    (
                        ...
                        f"Dr. {p['first_name']} {p['last_name']}", p["title"],
            ...
            for pt in patients:
                ...
                primary = next((d for d in diagnoses_by_patient.get(pt["patient_id"], []) if d["is_primary"]), None)
                last_session = max(
                ...
                extended = {
                    "demographics": {
                        "city": pt["city"], "state": pt["state"],
                        "date_of_birth": pt["date_of_birth"],
                    },
                    "source_pdf": pdf_by_patient.get(pt["patient_id"], ""),
                    "sample_theme": theme,
                }
                name = " ".join(part for part in (pt["first_name"], pt["middle_name"], pt["last_name"]) if part)
                conn.execute(
                    """
                    ...
                    """,
                    (
                        pid, name, pt["age_computed"], pt["gender"], pt["date_of_birth"], None,
                        _severity_to_risk_level(primary["severity"] if primary else None),
                        primary["diagnosis_name"] if primary else "",
                        provider_name(pt["provider_id"]), last_session,
                        1.0,
                        sum(1 for lr in lab_results if lr["patient_id"] == pt["patient_id"] and lr["is_abnormal"]),
                        "verified", json.dumps(extended), pt["smoking_status"], pt["alcohol_use"],
                        pt["bmi_computed"], _care_archetype(theme), "DEIDENTIFIED", "full", "high",
                        pt["provider_id"],
                    ),
                )
            ...
            for d in diagnoses:
                ...
                patient = next(p for p in patients if p["patient_id"] == d["patient_id"])
                conn.execute(
                    """
                    ...
                    """,
                    (
                        pid, d["icd_code"], d["diagnosis_name"], theme_by_patient.get(d["patient_id"], ""),
                        d["diagnosis_date"], d["status"], d["severity"], bool(d["is_primary"]),
                        provider_name(patient["provider_id"]), d["notes"],
                    ),
                )
            ...
            for m in medications:
                ...
                patient = next(p for p in patients if p["patient_id"] == m["patient_id"])
                conn.execute(
                    """
                    ...
                    """,
                    (
                        pid, m["drug_name"], f"{m['dosage_amount']} {m['dosage_unit']}", m["route"],
                        m["frequency"], m["start_date"], "active" if m["is_active"] else "stopped",
                        m["generic_name"], m["brand_name"], m["indication"],
                        provider_name(patient["provider_id"]), m["refills_remaining"], m["pharmacy_name"],
                    ),
                )
            ...
            for lr in lab_results:
                ...
                if panel_key not in panel_ids:
                    cur = conn.execute(
                        ...
                        """,
                        (pid, lr["panel_name"], lr["resulted_date"], lr["resulted_date"], lr["resulted_date"]),
                    )
                ...
                conn.execute(
                    ...
                    (
                        ...
                        pid, lr["resulted_date"], lr["test_name"], lr["test_name"], str(lr["result_value"]), lr["result_unit"],
                        f"{lr['reference_range_low']}-{lr['reference_range_high']}", _normalize_flag(lr["abnormal_flag"]),
                        panel_ids[panel_key], lr["reference_range_low"], lr["reference_range_high"],
                        bool(lr["is_abnormal"]), lr["result_status"],
                    ),
                )
            ...
            for v in vital_signs:
                ...
                bmi = round(v["weight_kg"] / (height_m ** 2), 1) if v["weight_kg"] and height_m else None
                conn.execute(
                    """
                    ...
                    """,
                    (
                        pid, v["recorded_at"], v["systolic_bp"], v["diastolic_bp"], v["heart_rate"],
                        v["respiratory_rate"], v["spo2_percent"], v["temperature_celsius"], v["weight_kg"],
                        bmi, v["pain_score"], v["blood_glucose_mgdl"],
                    ),
                )
            ...
            for pr in procedures:
                ...
                patient = next(p for p in patients if p["patient_id"] == pr["patient_id"])
                conn.execute(
                    """
                    ...
                    """,
                    (
                        pid, pr["procedure_date"], pr["procedure_name"], pr["cpt_code"], pr["outcome"],
                        provider_name(patient["provider_id"]), pr["facility_name"], pr["indication"], pr["status"],
                    ),
                )
                counts["procedures"] += 1
            ...
            for n in clinical_notes:
                ...
                patient = next(p for p in patients if p["patient_id"] == n["patient_id"])
                ...
                conn.execute(
                    ...
                    """,
                    (note_id, pid, n["note_date"], provider_name(patient["provider_id"]), n["note_type"], n["note_text"]),
                )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Ingest the live sample dataset into the real Capstone tenant DB.")
    parser.add_argument("--source", type=Path, default=Path(r"C:\Users\User\Downloads\New folder\live_samples_relational_seed.sqlite"))
    parser.add_argument("--db-path", type=Path, default=PROJECT_ROOT / "capstone.db")
    parser.add_argument("--uploads-dir", type=Path, default=PROJECT_ROOT / "uploads_capstone")
