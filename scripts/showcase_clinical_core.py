...
ICD10_BY_CODE = {
    code: (description, category) for code, description, category in ICD10_CATALOG
}
...
MEDICATION_CATALOG = [
    ...
    },
    {
        ...
        "dose": "100 mcg",
        "freq": "Once daily on empty stomach",
        "route": "Oral",
    ...
    },
    {
        ...
        "freq": "Once daily",
        "route": "Oral",
        "indication": "Inflammatory Condition",
    },
    {
        "name": "Gabapentin",
        "generic": "gabapentin",
    ...
    },
    {
        ...
        "brand": "Keytruda",
        "class": "PD-1 immunotherapy",
        "dose": "200 mg",
        "freq": "Every 3 weeks",
        "route": "IV",
        "indication": "Oncology Treatment",
    },
    {
        "name": "Erythropoetin Alfa",
    ...
    },
    {
        ...
        "dose": "800 mg",
        "freq": "Three times daily with meals",
        "route": "Oral",
        "indication": "CKD Mineral Management",
    },
    {
        "name": "Vitamin D3",
...
PAST_MEDICAL_CONDITIONS = [
    ...
    "Peptic ulcer disease (resolved)",
    "Deep vein thrombosis (resolved)",
...
TENANT_THEMES = {
    "primary": {
        "org": "Research Clinic",
        "facilities": [
            "Research Clinic Main Campus",
            "Riverside Community Hospital",
            "University Health System",
            "Lakeside Medical Center",
        ],
        "lab_facilities": [
            "Research Clinic Core Laboratory",
            "LabCorp — National Processing Center",
            "Quest Diagnostics — Regional Lab",
            "University Reference Laboratory",
        ],
    ...
    "demo2": {
        ...
        "lab_facilities": [
            "Northstar Central Laboratory",
            "ARUP Laboratories",
            "Mayo Clinic Laboratories",
            "Regional Diagnostics Alliance",
        ],
...
def _lab_value(
    rng: random.Random, test: dict[str, Any], conditions_text: str, anticoagulated: bool
    ...
    if "diabet" in lowered and name == "HbA1c":
        ...
        "hyperlipid" in lowered or "cholesterol" in lowered
    ) and name == "LDL Cholesterol":
...
def generate_clinical_note(
    ...
    display = (
        patient["deid_token"]
        if patient["privacy_class"] == "DEIDENTIFIED"
        else patient["name"]
    )
    symptom = rng.choice(
        [
    ...
    monitor = rng.choice(
        ["blood pressure", "blood glucose", "weight", "oxygen saturation"]
    )
    control = rng.choice(
        [
            "well-controlled",
...
def build_patient(
    ...
    height_cm = round(
        max(145.0, min(200.0, rng.gauss(175 if sex == "Male" else 162, 8))), 1
    )
    weight_kg = round(
        max(45.0, min(180.0, rng.gauss(88 if sex == "Male" else 74, 18))), 1
    )
    bmi = round(weight_kg / ((height_cm / 100.0) ** 2), 1)
    smoking = rng.choices(["Never", "Former", "Current"], weights=[62, 24, 14])[0]
    alcohol = rng.choices(
        ["None", "Social", "Moderate", "Heavy"], weights=[30, 38, 22, 10]
    )[0]
    ...
    risk_level = {"Low": "stable", "Moderate": "needs_review", "High": "high"}[
        risk_tier
    ]
    ...
    if privacy_class == "DEIDENTIFIED":
        ...
        employer = (
            f"{rng.choice(LAST_NAMES)} {rng.choice(['Group', 'Industries', 'Health', 'Logistics', 'Partners'])}"
            if occupation and rng.random() < 0.7
            else None
        )
    ...
    record: dict[str, Any] = {
        ...
        if privacy_class == "PII"
        else birth_date.replace(month=1, day=1).isoformat(),
        "gender_identity": rng.choices(
            ["Woman", "Man", "Nonbinary", "Not documented"], weights=[49, 48, 1, 2]
        )[0],
        ...
        "marital_status": rng.choices(
            ...
            weights=[46, 28, 14, 9, 3],
        )[0],
        "blood_type": rng.choices(
            ["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"],
            weights=[35, 6, 9, 2, 3, 1, 38, 6],
        )[0],
        "height_cm": height_cm,
        "weight_kg": weight_kg,
        "bmi": bmi,
        ...
        "deid_token": token,
        "consent_status": rng.choices(
            ["full", "limited_research", "treatment_only"], weights=[72, 18, 10]
        )[0],
        "record_quality": rng.choices(
            ["high", "mixed", "requires_reconciliation"], weights=[68, 24, 8]
        )[0],
        "sdoh_complexity": sdoh_complexity,
        "care_gaps": care_gaps,
        "source_systems": rng.sample(
            ["EHR", "Claims", "Lab", "Pharmacy", "Patient Portal", "RPM"],
            rng.randint(3, 5),
        ),
        "provider": provider,
        "care_team": care_team,
    ...
    for position, code in enumerate(codes[:condition_count]):
        ...
        if position == 0:
            ...
            severity = rng.choices(
                ["Mild", "Moderate", "Severe"], weights=[38, 42, 20]
            )[0]
        onset = rand_date(rng, date(2012, 1, 1), anchor - timedelta(days=60))
        gap_note = (
            f" Care gap: {care_gaps[0][1]}." if care_gaps and position == 0 else ""
        )
        conditions.append(
            {
                "code": code,
                ...
                "notes": (
                    f"Archetype={archetype}; risk={risk_tier}; quality={record['record_quality']}.{gap_note}"
                    if position == 0
                    else None
                ),
                "resolved_date": rand_date(rng, onset, anchor).isoformat()
                if status == "Resolved"
                else None,
    ...
    anticoagulated = any(
        "fibrillation" in item["name"].lower() or "thromb" in item["name"].lower()
    ...
    def _force(match: str, pool_filter) -> None:
        if match in lowered:
            candidates = [
                ...
                if pool_filter(med) and med not in forced
            ]
            if candidates:
    ...
    if anticoagulated:
        _force(
            "",
            lambda med: (
                "Anticoagulant" in med["class"] or "antagonist" in med["class"].lower()
            ),
        )
        ...
        medications.append(
            {
                ...
                "end_date": rand_date(rng, start, anchor).isoformat()
                if not is_active
                else None,
        ...
        allergies.append(
            {
                ...
                "reaction": reaction,
                "severity": severity,
                "onset_date": rand_date(rng, date(1995, 1, 1), anchor).isoformat()
                if rng.random() < 0.7
                else None,
        ...
        record.update(
            {
                ...
                "contacts": [],
                "social": {},
                "completeness": round(rng.uniform(0.62, 0.98), 2),
                "open_tasks": len(care_gaps),
                "key_metrics": {},
                "last_session_date": (
                    anchor - timedelta(days=rng.randint(0, lookback_days))
                ).isoformat(),
    ...
    base_hr = (
        rng.randint(72, 102)
        if archetype in ("respiratory", "complex_multimorbidity")
        else rng.randint(60, 88)
    )
    base_spo2 = (
        rng.uniform(90.0, 96.0)
        if archetype == "respiratory"
        else rng.uniform(94.5, 99.0)
    )
    ...
    while len(vitals) < n_vitals:
        ...
        measured = datetime.combine(
            anchor - timedelta(days=day_offset), datetime.min.time()
        ).replace(hour=rng.randint(7, 18), minute=rng.choice([0, 15, 30, 45]))
        glucose_base = (
            rng.uniform(125.0, 250.0)
            if "diabet" in lowered
            else rng.uniform(72.0, 150.0)
        )
        vitals.append(
            {
                ...
                "sbp": int(
                    max(
                        88, min(200, base_sbp + trend * progress + rng.randint(-13, 13))
                    )
                ),
                "dbp": int(
                    max(
                        52,
                        min(
                            118, base_dbp + trend * progress * 0.5 + rng.randint(-9, 9)
                        ),
                    )
                ),
                ...
                "spo2": round(
                    max(87.0, min(100.0, base_spo2 + rng.uniform(-1.8, 1.0))), 1
                ),
                "temp": round(
                    max(35.6, min(39.4, base_temp + rng.uniform(-0.4, 0.5))), 1
                ),
                "weight": round(max(42.0, weight_kg + rng.uniform(-2.5, 2.5)), 1),
                "bmi": bmi,
                "pain": rng.choices(
                    range(11), weights=[22, 11, 11, 15, 13, 9, 8, 5, 3, 2, 1]
                )[0],
    ...
    for panel_name in chosen:
        ...
        eligible = [
            test
            for category in PANEL_DEFINITIONS[panel_name]
            for test in LABS_BY_CATEGORY.get(category, [])
        ]
        take = (
            min(len(eligible), rng.randint(5, 8))
            if len(eligible) > 5
            else len(eligible)
        )
        results = []
        for test in rng.sample(eligible, take):
            value, is_abnormal, flag, status = _lab_value(
                rng, test, conditions_text, anticoagulated
            )
            results.append(
                {
                    "test": test["name"],
        ...
        panels.append(
            {
                ...
                "resulted_date": (
                    collected + timedelta(days=rng.randint(0, 3))
                ).isoformat(),
    ...
    for _ in range(rng.randint(2, 4)):
        ...
        status = rng.choices(
            ["Completed", "Scheduled", "Cancelled"], weights=[76, 14, 10]
        )[0]
        when = (
            rand_date(rng, history_start, anchor)
            if status != "Scheduled"
            else rand_date(rng, anchor, anchor + timedelta(days=90))
        )
        procedures.append(
            {
                "name": description,
                ...
                "outcome": rng.choice(
                    [
                        ...
                        "Completed without adverse events",
                        "Tolerated well",
                    ]
                )
                if status == "Completed"
                else None,
                "duration_minutes": rng.randint(15, 180)
                if status == "Completed"
                else None,
                "anesthesia": rng.choice(
                    ["None", "Local anesthesia", "General anesthesia", "Sedation", None]
                ),
                "status": status,
    ...
    if has_vaccine_gap:
        vaccine_pool = [
            vaccine for vaccine in vaccine_pool if vaccine[1] not in ("PPSV23", "PCV13")
        ]
    rng.shuffle(vaccine_pool)
    for vaccine_name, abbreviation, cvx, manufacturer, site, route in vaccine_pool[
        : rng.randint(4, 7)
    ]:
        ...
        immunizations.append(
            {
                ...
                "administered_by": provider["full_name"],
                "lot": rand_lot(rng),
                "manufacturer": manufacturer,
                "site": site,
                "route": route,
                "dose_number": rng.randint(1, 2),
                "series_complete": rng.random() < 0.7,
                "expiration": (
                    administered + timedelta(days=rng.randint(180, 730))
                ).isoformat(),
            }
        )
    ...
    for position in range(n_appointments):
        if position < n_appointments - 2:
            ...
            status = rng.choices(
                ...
                weights=noshow_weights,
            )[0]
        else:
            when = rand_date(
                rng, anchor + timedelta(days=1), anchor + timedelta(days=180)
            )
            status = "Scheduled"
    ...
    record["medical_history"] = [
        {
            "condition": condition,
            "onset_year": rng.randint(1985, 2018),
    ...
    record["surgical_history"] = [
        {
            ...
            "cpt": code,
            "date": rand_date(
                rng, date(1995, 1, 1), anchor - timedelta(days=90)
            ).isoformat(),
            "facility": rng.choice(theme["facilities"]),
            "surgeon": providers[(index + offset) % len(providers)]["full_name"],
            "anesthesia": anesthesia,
            "outcome": "Successful, patient tolerated procedure well",
            "indication": "Elective surgical intervention for indicated condition",
        }
        for offset, (name, code, anesthesia) in enumerate(
            surgical_pool[: rng.randint(0, 3)]
        )
    ]
        ...
        family.append(
            {
                "relation": relation,
                "condition": condition,
                "age_of_onset": rng.randint(35, 75)
                if (deceased or rng.random() < 0.6)
                else None,
                "is_deceased": deceased,
                "cause_of_death": condition if deceased else None,
            }
        )
    ...
    record["social"] = {
        "housing": rng.choices(
            ["stable housing", "temporary housing", "housing insecurity"],
            weights=[45, 33, 22] if sdoh_complexity else [88, 9, 3],
        )[0],
        "transportation": rng.choices(
            ["reliable", "limited", "requires assistance"],
            weights=[38, 40, 22] if sdoh_complexity else [80, 15, 5],
        )[0],
        "food_security": rng.choices(
            ["secure", "intermittent insecurity", "high insecurity"],
            weights=[42, 38, 20] if sdoh_complexity else [86, 11, 3],
        )[0],
        "financial_strain": rng.choices(
            ["low", "moderate", "high"],
            weights=[18, 40, 42] if sdoh_complexity else [58, 32, 10],
        )[0],
        "living_situation": rng.choice(
            ["alone", "with spouse", "with family", "assisted living"]
        ),
        "packs_per_day": round(rng.uniform(0.5, 2.0), 1)
        if smoking in ("Current", "Former")
        else None,
        "smoking_years": rng.randint(3, 40)
        if smoking in ("Current", "Former")
        else None,
        ...
        "employment": rng.choice(
            ["Unemployed", "Disabled", "Employed part-time", "Retired"]
            if sdoh_complexity
    ...
    for position in range(n_policies):
        coverage = rng.choices(
            ["HMO", "PPO", "EPO", "HDHP", "Medicare", "Medicaid", "Self-Pay"],
            weights=[24, 32, 7, 7, 16, 10, 4],
        )[0]
        ...
        insurance.append(
            {
                ...
                "end_date": None
                if rng.random() < 0.85
                else (start + timedelta(days=rng.randint(365, 730))).isoformat(),
                "deductible": float(
                    rng.choice([250, 500, 1000, 1500, 2000, 3000, 5000])
                ),
    ...
    for _ in range(rng.randint(3, 5)):
        ...
        if abnormal:
            ...
            lab_sentence = (
                f"{lab_ref['test']} returned {lab_ref['value']} {lab_ref['unit']} "
                f"({lab_ref['status']}; reference {lab_ref['low']}-{lab_ref['high']})."
            )
        else:
            lab_sentence = (
                "All recent laboratory values fell within their reference ranges."
            )
        notes.append(
            {
                ...
                "text": generate_clinical_note(
                    rng, record, primary, med["name"], lab_sentence
                ),
                "signed_at": datetime.combine(note_date, datetime.min.time())
                .replace(hour=rng.randint(8, 20), minute=rng.choice([0, 15, 30]))
                .isoformat(),
            }
        )
    ...
    quality_completeness = {
        "high": (0.86, 0.99),
        "mixed": (0.72, 0.9),
        "requires_reconciliation": (0.58, 0.78),
    }[record["record_quality"]]
    record["completeness"] = round(rng.uniform(*quality_completeness), 2)
    record["open_tasks"] = len(care_gaps) + (
        rng.randint(1, 3) if risk_tier == "High" else rng.randint(0, 1)
    )
    record["ai_review_status"] = (
        "needs_review"
        if (risk_level != "stable" and rng.random() < 0.75)
        else "verified"
    )
    record["last_session_date"] = (
        anchor - timedelta(days=rng.randint(0, min(120, lookback_days)))
    ).isoformat()
    return record
