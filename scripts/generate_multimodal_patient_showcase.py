...
def _patient_from_profile(profile: dict[str, Any], rng: random.Random, demo_platform: str) -> dict[str, Any]:
    """Create focus patient metadata from the shared enterprise blueprint."""

    meds = [
        {"name": item["name"], "dose": f"{item['dose']} {item['frequency']}", "status": item["status"]}
        for item in profile.get("medications", [])[:5]
    ]
    allergies = [item["allergen"] for item in profile.get("allergies", [])]
    care_gaps = [
        {"type": gap[0], "description": gap[1], "priority": "high" if profile.get("risk_tier") == "High" else "medium", "status": "open"}
...
def _timeline(patient_id: str, rng: random.Random, event_count: int, patient: dict[str, Any] | None = None) -> list[dict[str, Any]]:
...
def _notes(patient_id: str, rng: random.Random, count: int, patient: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    for index in range(1, count + 1):
        ...
        review_focus = patient.get("review_focus", "longitudinal synthesis") if patient else "longitudinal synthesis"
        notes.append(
            {
                "note_id": f"NOTE-{patient_id}-{index:03d}",
                "date": (date(2026, 6, 24) - timedelta(days=index * 5)).isoformat(),
                "author": patient["assigned_clinician"] if patient and index % 3 else rng.choice(CLINICIANS),
...
def _comparators(bundle_index: int, count: int, rng: random.Random, patient_prefix: str = "PT-D", demo_platform: str = "primary") -> list[dict[str, Any]]:
...
def _plotly_specs(patient_id: str, labs: list[dict[str, Any]], images: list[dict[str, Any]], comparators: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "image_modalities": {
            "data": [{"type": "bar", "x": list(modality_counts), "y": list(modality_counts.values()), "marker": {"color": [CHART_PALETTE[index % len(CHART_PALETTE)] for index in range(len(modality_counts))]}}],
            "layout": {"title": "Image evidence by modality"},
        },
        "comparator_risk": {
            "data": [{"type": "pie", "labels": list(risk_counts), "values": list(risk_counts.values()), "marker": {"colors": [RISK_COLORS.get(label, CHART_PALETTE[0]) for label in risk_counts]}}],
...
def _write_charts(bundle_dir: Path, patient_id: str, labs: list[dict[str, Any]], comparators: list[dict[str, Any]]) -> list[str]:
    ...
    for metric in ("tumor_size_cm", "cea_ng_ml", "egfr"):
        ...
        plt.plot([row["date"] for row in rows], [row["value"] for row in rows], label=metric, color=METRIC_COLORS[metric])
    ...
    plt.bar(list(counts), list(counts.values()), color=[RISK_COLORS[risk] for risk in counts])
...
def _write_pdf_corpus(
    ...
    for index in range(1, pdf_count + 1):
        ...
        with PdfPages(path) as pdf:
            ...
            page.axis("off")
            page.text(0.08, 0.94, f"{patient.get('organization', 'Clinical')} Multimodal Evidence Packet", fontsize=16, weight="bold")
            ...
            chart.plot([row["date"] for row in trend_rows], [row["value"] for row in trend_rows], marker="o", color=METRIC_COLORS.get(metric, CHART_PALETTE[0]))
...
def build_bundle(
    ...
    profile = core.build_patient(bundle_index, seed, patient_prefix, date(2026, 7, 5), 4, provider_rows, demo_platform)
    patient_id = profile["patient_id"]
    ...
    comparator_rows = _comparators(bundle_index, comparators, rng, patient_prefix, demo_platform)
...
def build_bundle_from_template_item(bundle_index: int, item: dict[str, Any], output: Path, rng: random.Random) -> dict[str, Any]:
    ...
    for idx, note in enumerate(item.get("notes", []), 1):
        notes.append({
            ...
            "note_type": note.get("note_type") or note.get("type") or "Progress Note",
            "text": note.get("text", ""),
            "vector_chunk_id": note.get("vector_chunk_id") or f"VEC-{patient_id}-{idx:03d}",
    ...
    for idx, event in enumerate(item.get("timeline", []), 1):
        timeline.append({
            ...
            "event_type": event.get("event_type") or event.get("type") or "Encounter",
    ...
    for idx, img in enumerate(item.get("images", []), 1):
        images.append({
            ...
            "gcs_uri": img.get("gcs_uri") or f"gs://clinical-data/{patient_id}/images/img-{idx}.png",
    ...
    for idx, comp in enumerate(item.get("comparator_cohort", []), 1):
        comparator_rows.append({
            ...
            "primary_diagnosis": comp.get("primary_diagnosis") or "Metastatic NSCLC",
            "assigned_clinician": comp.get("assigned_clinician") or "Dr. Sarah Miller",
        })
    ...
    pdf_documents = _write_pdf_corpus(bundle_dir, patient_data, labs, images, notes, timeline, pdf_count)
    knowledge_base_documents = _write_knowledge_base_corpus(bundle_dir, patient_data, labs, notes, timeline, kb_count)

    citations = [
        {"citation_id": document["document_id"], "source_id": document["path"], "kind": "document", "excerpt": "Knowledge-base upload file."}
        for document in knowledge_base_documents
    ] + [
        {"citation_id": document["document_id"], "source_id": document["path"], "kind": "pdf", "excerpt": "Uploadable PDF packet."}
...
def _app_contract(bundles: list[dict[str, Any]], demo_platform: str) -> dict[str, Any]:
    ...
    pending_review = sum(1 for bundle in bundles if bundle.get("risk_level") in {"high", "needs_review"})
    stored_files = totals.get("pdf_documents", 0) + totals.get("knowledge_base_documents", 0)
    avg_completeness = round(
        sum(float(bundle.get("completeness", 0.9)) for bundle in bundles) / max(1, len(bundles)) * 100
    )
    return {
        ...
        "agentMonitoringSeed": [
            {"pipeline": "qa", "agent": "context_assembly_agent", "runs": len(patients) * 3, "avgConfidence": 0.92, "failureRate": 0.0, "reviewRate": round(pending_review / max(1, len(patients)), 2), "avgDurationMs": 620},
            {"pipeline": "qa", "agent": "evidence_retrieval_agent", "runs": len(patients) * 3, "avgConfidence": 0.9, "failureRate": 0.0, "reviewRate": 0.0, "avgDurationMs": 1380},
            {"pipeline": "qa", "agent": "image_evidence_agent", "runs": len(patients) * 2, "avgConfidence": 0.88, "failureRate": 0.0, "reviewRate": 0.0, "avgDurationMs": 1660},
            {"pipeline": "qa", "agent": "citation_builder_agent", "runs": len(patients) * 3, "avgConfidence": 0.93, "failureRate": 0.0, "reviewRate": 0.0, "avgDurationMs": 840},
            {"pipeline": "qa", "agent": "answer_synthesis_agent", "runs": len(patients) * 3, "avgConfidence": 0.91, "failureRate": 0.0, "reviewRate": 0.0, "avgDurationMs": 1810},
        ],
    }
...
def generate(
    ...
    if template_path:
        ...
        bundles = [
            build_bundle(index, output, rng, days, comparators, pdfs_per_bundle, kb_docs_per_bundle, demo_platform, patient_prefix, seed, providers)
...
def main() -> None:
    ...
    manifest = generate(args.output, args.bundle_count, args.days, args.comparators, args.seed, args.pdfs_per_bundle, args.kb_docs_per_bundle, args.template, args.demo_platform, args.patient_prefix)
