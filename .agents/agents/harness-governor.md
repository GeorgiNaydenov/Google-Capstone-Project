...
1. Read `.agents/memory/project.md` and `.agents/memory/handoff-protocol.md`.
...
Own `.agents/`, `.agents/`, `CLAUDE.md`, `AGENTS.md`, and harness scripts.
Keep `.claude` canonical and use `scripts/sync_harness.py` for portable mirror.
Never write durable facts into `.agents/state/`; state is local and ephemeral.
