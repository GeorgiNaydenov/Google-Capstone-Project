...
import { api } from "../api";
import { AuditDetailModal, AuditTimeline, Card, CompletenessBadge, ConfidenceMeter, DenseTable, EmptyState, ErrorState, JsonViewer, KpiStrip, LoadingState, RiskBadge, StatusBadge } from "../components";
...
export function PatientSearch({ queue = false }: { queue?: boolean }) {
  ...
  return <>
    ...
    <div className="search-layout">
      ...
      <Card title={queue ? "Queue intelligence" : "AI search explanation"} className="intelligence-panel"><span className="agent-kicker">Retrieval Agent</span><h3>{queue ? `${rowStats.high} urgent cases surfaced` : "Hybrid retrieval active"}</h3><p>The result set combines patient demographics, structured diagnoses, clinical notes, imaging metadata, and vector similarity within your authorized scope.</p><dl><div><dt>Structured matches</dt><dd>{rows.length}</dd></div><div><dt>Image evidence</dt><dd>{rowStats.imageEvidence}</dd></div><div><dt>Review required</dt><dd>{rowStats.reviewRequired}</dd></div></dl><button className="button primary full" onClick={() => navigate("/app/qa")}>Ask about these results</button></Card>
    </div>
  </>;
}
