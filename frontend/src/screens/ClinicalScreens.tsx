...
export function PatientSearch({ queue = false }: { queue?: boolean }) {
  ...
  }), [patientState.data, risk, review, completeness, quickFilters]);
...
function RecordList({ items }: { items: string[] }) { return <div className="record-list">{items.map((item, index) => <button key={item}><span>{index + 1}</span><span><strong>{item}</strong><small>Verified clinical record. Source available</small></span><b>Open source</b></button>)}</div>; }
