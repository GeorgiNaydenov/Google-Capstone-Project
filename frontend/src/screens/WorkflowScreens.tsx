...
import { api } from "../api";
import { AgentMeta, AgentStepper, Card, ChartPanel, ConfidenceMeter, ConfirmDialog, DenseTable, EmptyState, ErrorState, JsonViewer, LoadingState, ReviewChecklist, SourceViewer, SqlPreview, StatusBadge, useToast } from "../components";
...
function PipelineBand({ agents, run }: { agents: string[]; run: AgentRun | null }) {
  ...
  return <div className="pipeline-band">{agents.map((agent, index) => {
    const status = stepStatuses[index] ?? (run ? run.status : index === 0 ? "queued" : "waiting");
...
export function Extraction() {
  ...
  const [wizardStep, setWizardStep] = useState<"source" | "execution" | "output" | "review">("source");
  ...
  useEffect(() => {
    ...
    if (!first) return;
    if (syntheticSource && sourceOptions.some(option => option.assetId === syntheticSource.assetId)) return;
    setSyntheticSource(first);
    setPatientId(first.patientId);
    setPreview(first.previewUrl);
    setExtracted(extractedFromSource(first));
  ...
  const start = async () => {
    if (demoMode) {
      ...
      try { setExtracted(extractedFromSource(source)); const next = await api.runSyntheticExtraction(source.assetId, patientId || source.patientId); setRun(next); record(next); setWizardStep("execution"); }
    ...
    setBusy(true); setError("");
    try { const asset = await api.upload(file, patientId); if (asset.extracted) setExtracted(asset.extracted as Record<string, unknown>); const next = await api.runExtraction(asset.assetId, patientId); setRun(next); record(next); setWizardStep("execution"); }
  ...
  const wizardSteps = [
    ...
    { id: "output", label: "Output", detail: run ? `${fieldRows.length} fields` : "Awaiting run", disabled: !run },
    { id: "review", label: "Review", detail: run?.status === "completed" ? "Synced" : "Human gate", disabled: !run },
  ] as const;
  return <>
...
function EvidenceCitationList({ evidence, onOpen }: { evidence: Evidence[]; onOpen: (item: Evidence) => void }) {
  if (!evidence.length) return null;
  return <div className="inline-citations" aria-label="Inline evidence citations">
    <h4>Evidence cited in this answer</h4>
...
export function PatientQa() {
  ...
  useEffect(() => {
    ...
    if (list.length && !list.some(item => item.id === patientId)) setPatientId(list[0].id);
  }, [patientList.data]);
  useRunPolling(run, setRun);
  ...
  return <>
    ...
    <PipelineBand agents={qaPipeline?.agents ?? ["context_assembly_agent", "evidence_retrieval_agent", "image_evidence_agent", "citation_builder_agent", "answer_synthesis_agent", "qa_audit_agent"]} run={run}/>
...
const databaseExamples = ["Count patients by risk level", "Which diabetic patients have an HbA1c above 9 percent?", "Which anticoagulated patients have no recent INR result?", "Which patients aged 65 and older are missing pneumococcal vaccination?", "Which patients are on 8 or more active medications?", "How does appointment no-show rate vary with housing stability?", "Show open care gaps by priority and owner"];
...
function stepName(run: AgentRun, fallback: string, index: number) {
  return run.steps?.[index]?.name ?? fallback;
}
...
export function DatabaseIntelligence() {
  ...
  const preview = async () => {
    ...
    try {
      ...
      if (draft.result?.safe === false) { setRun(draft); appendTurn(draft.id, submitted, draft); return; }
      const executed = await api.executeSql(draft.id);
      setRun(executed);
      appendTurn(executed.id, submitted, executed);
      setQuestion("");
  ...
  return <>
    ...
    <PipelineBand agents={dbPipeline?.agents ?? ["schema_discovery_agent", "nl_to_sql_agent", "sql_validator_agent", "query_executor_agent", "insight_chart_agent", "audit_agent"]} run={run}/>
