...
function PipelineBand({ agents, run }: { agents: string[]; run: AgentRun | null }) {
  return <div className="pipeline-band">{agents.map((agent, index) => <div key={agent} className={run ? "complete" : index === 0 ? "ready" : "waiting"}><span>{run ? "ok" : index + 1}</span><strong>{agent.replaceAll("_", " ").replace(" agent", "")}</strong><small>{run ? "completed" : index === 0 ? "ready" : "waiting"}</small></div>)}</div>;
}
...
export function DatabaseIntelligence() {
  ...
  return <>
    ...
    <div className="database-workspace">
      <aside>
        <Card title="Schema explorer">{schemaTables.map(table => <details key={table.name} open={table.name === "patients"}><summary>{table.name}<span>table</span></summary><code>{table.columns}</code></details>)}</Card>
        <InlineDiagram id="23-clinical-database-erd" title="Database schema architecture"/>
        <Card title="Query policy"><ul className="policy-list"><li>SELECT statements only</li><li>Recognized tables only</li><li>No system catalog access</li><li>Explicit execution approval</li><li>Every query audited</li></ul></Card>
      </aside>
