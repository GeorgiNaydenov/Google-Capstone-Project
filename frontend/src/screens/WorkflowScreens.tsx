...
function WorkflowHead({ number, title, detail, mode = "local" }: { number: number; title: string; detail: string; mode?: string }) {
  return <header className="page-head workflow-head"><div><span className="eyebrow accent">GUIDED WORKFLOW {number}</span><h1>{title}</h1><p>{detail}</p></div><div className="runtime-state"><span className="pulse"/><div><strong>Clinical AI Kit</strong><small>{mode === "live" ? "Live AI execution" : "Deterministic demo mode"}</small></div></div></header>;
}
...
export function Extraction() {
  ...
  const agents = extractionPipeline?.agents ?? ["quality_assessor_agent", "ocr_processor_agent", "vision_analyzer_agent", "clinical_structuring_agent", "extraction_critic_agent", "extraction_refiner_agent", "clinical_review_request_agent"];
...
export function PatientQa() {
  ...
  const qaAgents = qaPipeline?.agents ?? ["qa_request_validation_agent", "context_assembly_agent", "evidence_retrieval_agent", "image_evidence_agent", "citation_builder_agent", "answer_synthesis_agent", "qa_audit_agent", "qa_response_agent"];
...
export function DatabaseIntelligence() {
  ...
  const dbAgents = dbPipeline?.agents ?? ["schema_discovery_agent", "nl_to_sql_agent", "sql_validator_agent", "sql_preview_approval_agent", "query_executor_agent", "insight_chart_agent"];
