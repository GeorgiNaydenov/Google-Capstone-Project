...
export function AgentConfiguration() {
  ...
  const renderSection = () => {
    ...
    if (currentPipeline) return <><Card title={currentPipeline.name}><div className="pipeline-config">{currentPipeline.agents.map((agent, index) => <div key={agent}><span>{index + 1}</span><div><strong>{agent.replaceAll("_", " ")}</strong><small>Google ADK specialist. Tool access governed.</small></div><label className="toggle"><input type="checkbox" checked={!disabledAgents.includes(agent)} onChange={() => setDisabledAgents(current => current.includes(agent) ? current.filter(item => item !== agent) : [...current, agent])}/><span>Enabled</span></label><button className="button subtle" onClick={() => openLogs(currentPipeline.id)}>Logs</button></div>)}</div></Card><Card title="Pipeline controls"><label>Confidence threshold<div className="range-row"><input type="range" min="0" max="100" value={config.reviewThreshold} onChange={event => update("reviewThreshold", Number(event.target.value))}/><strong>{config.reviewThreshold}%</strong></div></label><div className="locked-policy"><span><strong>Require human review</strong><small>Gate consequential persistence and execution</small></span><StatusBadge tone="review">Required</StatusBadge></div></Card></>;
