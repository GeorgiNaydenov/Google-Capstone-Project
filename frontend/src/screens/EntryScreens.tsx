...
const docsGuides: DocsGuide[] = [
  {
    ...
    prerequisites: ["Python 3.11+", "uv — dependencies are declared in pyproject.toml and hash-pinned in uv.lock (fastapi and uvicorn included)"],
    steps: [
      "Open a terminal and cd into the repo root — the folder that directly contains capstone_agent/, clinical_app/, frontend/, and pyproject.toml.",
      "Install everything in one step: uv sync — this creates .venv and installs the exact dependency set resolved in uv.lock.",
  ...
  },
  {
    ...
    prerequisites: ["None to read the pre-rendered pages — they already ship in frontend/public/documentation/project-wiki/", "Obsidian desktop app (free, obsidian.md) only if you want the real vault with backlinks, graph view, and search", "To rebuild the static pages after editing Project Wiki/*.md: Python 3.11+ and uv sync (markdown-it-py is part of the locked dependency set)"],
    steps: [
      ...
      "After editing any Project Wiki/*.md file, regenerate the static pages so the /documentation site matches: uv run python scripts/build_docs_site.py from the repo root (uv sync already installed markdown-it-py).",
    ],
  ...
  },
  {
    ...
    prerequisites: ["None to just read it — already committed under frontend/public/documentation/llm-wiki/", "To rebuild after editing wiki/*.md: Python 3.11+ and uv sync (markdown-it-py is part of the locked dependency set)"],
    steps: [
      ...
      "After editing wiki/*.md or Project Wiki/*.md, rebuild both wikis in one pass: uv run python scripts/build_docs_site.py from the repo root.",
    ],
